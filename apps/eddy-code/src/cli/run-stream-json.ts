/**
 * Compatible stream-json I/O adapter.
 *
 * Reads stdin in a stream-json dialect, processes messages through the
 * EddyCode agent, and writes stdout in the same dialect. This lets cc-connect
 * and other stdio clients use EddyCode as a drop-in agent backend.
 */
...
class StreamJsonWriter {
  ...
  /** Emit a resume hint as a meta message (same format as stream-json clients). */
  emitResumeHint(sessionId: string): void {
    this.writeJson({
      role: "meta",
      type: "session.resume_hint",
      session_id: sessionId,
      command: `eddy -r ${sessionId}`,
      content: `Resume this session: eddy -r ${sessionId}`,
    });
  }
...
/** Extract plain text from a stream-json-style user message. */
...
import type { PermissionMode } from "@eddy-code/sdk";
...
export async function runStreamJson(opts: StreamJsonOptions): Promise<void> {
  ...
  const harness = new EddyHarness({
    homeDir,
    identity: createEddyCodeHostIdentity("dev"),
    uiMode: "print",
    skillDirs: opts.skillsDirs,
  });

  const writer = new StreamJsonWriter((line) => {
  ...
  if (opts.appendSystemPromptFile) {
    try {
      appendSystemPromptParts.push(await readFile(opts.appendSystemPromptFile, "utf-8"));
    } catch (error) {
  ...
  if (opts.appendSystemPrompt) {
    appendSystemPromptParts.push(opts.appendSystemPrompt);
  }
  const appendSystemPrompt = appendSystemPromptParts.length > 0
    ? appendSystemPromptParts.join("\n")
    : undefined;

  // cc-connect passes --append-system-prompt or --append-system-prompt-file with instructions for
  ...
  const agentsMdPath = join(workDir, ".eddy-code", "AGENTS.md");
  ...
  if (appendSystemPrompt) {
    ...
    await mkdir(join(workDir, ".eddy-code"), { recursive: true });
    const sendHint =
  ...
  try {
    ...
    for await (const msg of readStdinMessages()) {
      ...
      unsubscribe = session!.onEvent((event) => {
        ...
        // Translate EddyCode events to compatible stream-json dialect.
