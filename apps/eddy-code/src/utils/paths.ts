...
import {
  EDDY_CODE_DATA_DIR_NAME,
  EDDY_CODE_HOME_ENV,
  EDDY_CODE_INPUT_HISTORY_DIR_NAME,
  EDDY_CODE_LOG_DIR_NAME,
  EDDY_CODE_UPDATE_DIR_NAME,
  EDDY_CODE_UPDATE_STATE_FILE_NAME,
} from '#/constant/app';

/**
 * Return the root data directory for Eddy Code.
 *
 * Priority: `EDDY_CODE_HOME` env var > `~/.eddy-code`.
 */
export function getDataDir(): string {
  const envDir = process.env[EDDY_CODE_HOME_ENV];
  if (envDir) {
    return envDir;
  }
  return join(homedir(), EDDY_CODE_DATA_DIR_NAME);
}
...
export function getLogDir(): string {
  return join(getDataDir(), EDDY_CODE_LOG_DIR_NAME);
}
...
export function getUpdateStateFile(): string {
  return join(getDataDir(), EDDY_CODE_UPDATE_DIR_NAME, EDDY_CODE_UPDATE_STATE_FILE_NAME);
}
...
export function getInputHistoryFile(workDir: string): string {
  ...
  return join(getDataDir(), EDDY_CODE_INPUT_HISTORY_DIR_NAME, `${hash}.jsonl`);
}
