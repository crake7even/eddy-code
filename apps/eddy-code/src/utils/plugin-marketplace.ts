...
import {
  ...
  EDDY_CODE_PLUGIN_MARKETPLACE_URL_ENV,
} from '#/constant/app';
...
export async function loadPluginMarketplace(
  ...
  const location = resolveMarketplaceLocation(
    options.source ?? process.env[EDDY_CODE_PLUGIN_MARKETPLACE_URL_ENV] ?? EDDY_CODE_PLUGIN_MARKETPLACE_URL,
    options.workDir,
  );
...
function resolveMarketplaceLocation(source: string, workDir: string): MarketplaceLocation {
  ...
  if (trimmed.length === 0) {
    throw new Error(`${EDDY_CODE_PLUGIN_MARKETPLACE_URL_ENV} 不能为空.`);
  }
