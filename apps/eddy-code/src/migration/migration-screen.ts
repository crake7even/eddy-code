 ...
 * conventions in `apps/eddy-code/src/tui/components/dialogs/choice-picker.ts`.
 *
...
} from '@eddy-code/migration-legacy';
...
export interface MigrationScreenOptions {
   ...
   * now/later/never gate — used by the explicit `eddy migrate` command, where
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  constructor(opts: MigrationScreenOptions) {
    ...
    if (opts.skipDecisionStep === true) {
      // Explicit `eddy migrate`: the now/later/never gate is meaningless, so
  ...
  private renderResult(width: number): string[] {
    ...
    if (this.migrationFailed) {
      ...
      lines.push('');
      lines.push(chalk.hex(colors.text)(' 可稍后运行 "eddy migrate" 重试。'));
      lines.push('');
      lines.push(chalk.hex(colors.textMuted)(' ⏎ 继续进入 Eddy Code'));
    ...
    if (r !== undefined) {
      ...
      if (r.notices.detectedPlugins.length > 0) {
        lines.push(
          chalk.hex(colors.warning)(
            `  ⚠ ${r.notices.detectedPlugins.length} 个 eddy-code 插件 — 暂不支持迁移`,
          ),
        );
      }
      // OAuth credentials are deliberately not migrated (refresh tokens cannot
      // safely be held by two installs at once). Eddy Code's normal auth flow
      ...
      // landed in a *.migrated-from-eddy-code.* sibling or that sessions failed.
      ...
      if (sum.config.wroteSiblingDueToConflict) {
        ...
        // migrated content went to `config.migrated-from-eddy-code.toml` and
        ...
        lines.push(
          chalk.hex(colors.warning)(
            '  ⚠ config.toml 无法解析 — 请查看 config.migrated-from-eddy-code.toml',
          ),
        );
      ...
      if (sum.config.wroteTuiSibling) {
        lines.push(
          chalk.hex(colors.warning)(
            '  ⚠ tui.toml 冲突 — 请检查 tui.migrated-from-eddy-code.toml',
          ),
        );
      }
      if (sum.mcp.wroteSiblingDueToConflict) {
        lines.push(
          chalk.hex(colors.warning)(
            '  ⚠ mcp.json 无法读取 — 请检查 mcp.migrated-from-eddy-code.json',
          ),
        );
      }
      ...
      lines.push('');
      lines.push(
        chalk.hex(colors.textMuted)(' 旧数据保留在 ~/.eddy-code/ — eddy-code 仍可正常使用。'),
      );
    }
    lines.push('');
    lines.push(chalk.hex(colors.textMuted)(' ⏎ 继续进入 Eddy Code'));
  ...
  private renderProgress(width: number): string[] {
    ...
    const lines: string[] = [
      ...
      chalk.hex(colors.primary).bold(' 正在从 eddy-code 迁移'),
      '',
    ];
  ...
  private renderAsk(width: number): string[] {
    ...
    const lines: string[] = [
      chalk.hex(colors.primary)('─'.repeat(width)),
      chalk.hex(colors.primary).bold(' 从 eddy-code 迁移'),
      '',
    ];
    if (this.phase === 'ask1') {
      lines.push(chalk.hex(colors.text)(' 发现已有的 eddy-code 安装:'));
