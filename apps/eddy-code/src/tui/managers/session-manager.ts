...
import { LLM_NOT_SET_MESSAGE, MAIN_AGENT_ID, NO_ACTIVE_SESSION_MESSAGE } from '../constant/eddy-tui';
...
/**
 * Interface exposing only the EddyTUI surface that SessionManager needs.
    ...
    if (isResumeStartup) {
      ...
      if (startup.sessionFlag !== undefined) {
        ...
        if (target.workDir !== workDir) {
          throw new Error(
            `会话 "${startup.sessionFlag}" 是在其他目录下创建的。\n  cd "${target.workDir}" && eddy -r ${startup.sessionFlag}`,
          );
        }
