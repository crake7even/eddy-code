...
import type { ApprovalRequest, ApprovalResponse } from '@eddy-code/sdk';
...
export class TranscriptController {
  ...
  commit(): void {
    ...
    if (process.env['EDDY_CODE_DEBUG'] === '1') {
  ...
  renderWelcome(): void {
    ...
    welcome.borderTitle = PRODUCT_NAME;
  ...
  toggleToolOutputExpansion(): void {
    const { state } = this.host;
    // Avoid the full-screen clear that pi-tui 0.78.1 issues when content shrinks
    // after collapsing a tool card.
  ...
  // Package-visible helpers for EddyTUI to reach specific components.
