...
import { createEddyTUIThemeBundle, type EddyTUIThemeBundle } from './theme/bundle';
...
export function createTUIState(options: EddyTUIOptions): TUIState {
  const initialAppState = options.initialAppState;
  const theme = createEddyTUIThemeBundle(initialAppState.theme, options.resolvedTheme);
