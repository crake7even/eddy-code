...
export function runLoadingAnimation(theme: ResolvedTheme = 'dark'): Promise<void> {
  ...
  return new Promise((resolve) => {
    ...
    function render() {
      ...
      lines.push('')
      lines.push('')
      lines.push(centerPad(`${fg(...DIM_RGB)}按住 Ctrl+C 即可退出 Eddy Code${RESET}`, cols))

      while (lines.length < rows) lines.push('')

      stdout.write('\x1b[H')
      // A newline only moves the cursor; it does not erase content left by a
      // previous frame after the terminal is resized.
