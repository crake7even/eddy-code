...
import type { TokenUsage } from '@eddy-code/sdk';
...
export class ToolCallComponent extends Container {
   ...
   * `args.plan` is empty. `EddyTUI` calls `setPlanInfo` with
  ...
  // when EddyTUI routes a `subagent.event` with this tool call
   ...
   * this: plan-file mode leaves `args.plan` empty, so `EddyTUI` fetches
  ...
  // ── Subagent API (called by EddyTUI event routing) ───────────────
