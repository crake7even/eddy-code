/**
 * /update slash command — manually install the latest Eddy Code update.
 *
 * Runs `git pull + pnpm install + pnpm -r build` in the source install dir,
...
import { getDefaultSourceInstallDir, getSourceInstallDir } from '#/cli/update/source';
...
export async function handleUpdateCommand(host: SlashCommandHost): Promise<void> {
  ...
  const installDir = getSourceInstallDir();
  if (installDir === null) {
    host.showError(
      '无法定位 Eddy Code 源码安装目录。请手动进入安装目录后执行：' +
      `git pull origin main && pnpm install && pnpm -r build。` +
      `默认源码安装目录通常是 ${getDefaultSourceInstallDir()}。`,
    );
    return;
  }
