...
import type { Session } from '@eddy-code/sdk';

import { detectInstallSource } from '#/cli/update/source';
import { detectShellEnvironment } from '#/utils/process/shell-env';
import { toTerminalHyperlink } from '#/utils/terminal-hyperlink';
import { LLM_NOT_SET_MESSAGE, NO_ACTIVE_SESSION_MESSAGE } from '../constant/eddy-tui';
...
export async function handleForkCommand(host: SlashCommandHost, args: string): Promise<void> {
  ...
  try {
    await host.switchToSession(
      forked,
      `Session forked (${forked.id}). To return to the original session: eddy -r ${session.id}`,
    );
  } catch (error) {
    const msg = formatErrorMessage(error);
...
export async function handleExportMdCommand(host: SlashCommandHost, args: string): Promise<void> {
  ...
  try {
    ...
    const defaultName = `eddy-export-${shortId}-${timestamp}.md`;
