import type { EddySlashCommand, SlashCommandAvailability } from './types';
...
export const BUILTIN_SLASH_COMMANDS = [
  ...
  },
  {
    name: 'plugin',
    aliases: ['plugins'],
    description: 'Eddy Code 插件中心：浏览、安装、卸载插件',
    priority: 111,
    availability: 'always',
  },
  {
  ...
  // ── 系统 ──
  {
    name: 'update',
    aliases: [],
    description: '手动更新 Eddy Code 到最新版本',
    priority: 95,
    availability: 'idle-only',
  },
  {
...
] as const satisfies readonly EddySlashCommand[];
...
export function findBuiltInSlashCommand(commandName: string): BuiltinSlashCommand | undefined {
  const commands = BUILTIN_SLASH_COMMANDS as readonly EddySlashCommand<BuiltinSlashCommandName>[];
...
export function sortSlashCommands(commands: readonly EddySlashCommand[]): EddySlashCommand[] {
