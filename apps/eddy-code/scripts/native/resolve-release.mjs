...
function main() {
  ...
  const hasEddyCode = packages.some(
    (pkg) => pkg.name === 'eddy-code',
  );

  // Native builds disabled for v0.3.0 initial release.
  // Flip to `hasEddyCode` when native artifact pipeline is ready.
  const shouldPublish = false;

  const tag = hasEddyCode && packages[0]?.version
