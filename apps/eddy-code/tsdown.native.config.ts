...
function buildTarget(): string {
  return process.env['EDDY_CODE_BUILD_TARGET'] ?? `${process.platform}-${process.arch}`;
}
...
export default defineConfig({
  ...
  define: {
    ...
    __EDDY_CODE_VERSION__: JSON.stringify(packageJson.version),
    __EDDY_CODE_CHANNEL__: JSON.stringify(process.env['EDDY_CODE_CHANNEL'] ?? ''),
    __EDDY_CODE_COMMIT__: JSON.stringify(process.env['EDDY_CODE_COMMIT'] ?? ''),
    __EDDY_CODE_BUILD_TARGET__: JSON.stringify(buildTarget()),
    __EDDY_CODE_NATIVE_BUNDLE__: 'true',
  },
  deps: {
