...
import { EddyTUI, type EddyTUIStartupInput, type TUIState } from "#/tui/eddy-tui";
import {
...
function makeStartupInput(
  cliOptions: Partial<EddyTUIStartupInput["cliOptions"]> = {},
  tuiConfig: Partial<EddyTUIStartupInput["tuiConfig"]> = {},
  resolvedTheme: EddyTUIStartupInput["resolvedTheme"] = "dark",
): EddyTUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
...
function makeHarness(session = makeSession(), overrides: Record<string, unknown> = {}) {
  return {
    getConfig: vi.fn(async () => ({
      models: {
        k2: { model: "eddy-code-v1", maxContextSize: 100 },
      },
    })),
...
function makeDriver(harness: ReturnType<typeof makeHarness>, input: EddyTUIStartupInput) {
  const driver = new EddyTUI(harness as never, input) as unknown as StartupDriver;
...
describe("EddyTUI startup", () => {
  ...
  it("passes the CLI model override when creating a fresh startup session", async () => {
    ...
    const driver = makeDriver(harness, makeStartupInput({ model: "eddy-code/k2.5" }));

    await expect(driver.init()).resolves.toBe(false);

    expect(harness.createSession).toHaveBeenCalledWith({
      workDir: "/tmp/proj-a",
      model: "eddy-code/k2.5",
  ...
  it("applies the CLI model override when resuming a startup session", async () => {
    ...
    const driver = makeDriver(
      harness,
      makeStartupInput({ continue: true, model: "eddy-code/k2.5" }),
    );

    await expect(driver.init()).resolves.toBe(true);

    expect(session.setModel).toHaveBeenCalledWith("eddy-code/k2.5");
    expect(driver.state.appState.model).toBe("eddy-code/k2.5");
  });
