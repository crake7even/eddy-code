...
describe('ToolCallComponent', () => {
  ...
  it('truncates a long file path from the head so the filename stays visible', () => {
    const longPath =
      'apps/eddy-code/src/tui/components/messages/tool-renderers/long-path/example/final-file.ts';
    const component = new ToolCallComponent(
      {
        id: 'call_long_path',
        name: 'Read',
    ...
    expect(out).not.toContain('apps/eddy-code/src/tui/components/messages/tool-renderers/long-pa…');
  });
  ...
  it('shows Read paths relative to the active workspace', () => {
    const component = new ToolCallComponent(
      {
        ...
        name: 'Read',
        args: { path: '/tmp/proj-a/apps/eddy-code/src/main.ts' },
      },
      {
    ...
    const out = strip(component.render(100).join('\n')).replace(/\\/g, '/');
    expect(out).toContain('已使用 Read (apps/eddy-code/src/main.ts)');
    expect(out).not.toContain('/tmp/proj-a/apps');
    const snapshot = component.getReadSnapshot();
    expect(snapshot).toBeDefined();
    expect((snapshot?.filePath ?? '').replace(/\\/g, '/')).toBe(
      'apps/eddy-code/src/main.ts',
    );
  });
  ...
  it('renders a single foreground subagent without the generic Agent tool header', () => {
    ...
    component.appendSubToolCall({
      ...
      name: 'Read',
      args: { path: 'apps/eddy-code/src/tui/utils/background-agent-status.ts' },
    });

    out = strip(component.render(120).join('\n'));
    expect(out).toContain('Explore Agent 运行中 (explore project xxx) · 1 个 tool · 10s');
    expect(out).toContain('正在使用 Read (apps/eddy-code/src/tui/utils/background-agent-status.ts)');
