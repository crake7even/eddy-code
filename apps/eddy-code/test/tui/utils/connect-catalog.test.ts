...
import { DEFAULT_CATALOG_URL, loadBuiltInCatalog } from '@eddy-code/sdk';
...
describe('built-in connect catalog injection', () => {
  ...
  it('embeds a generated catalog file through the tsdown define value', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'eddy-built-in-catalog-'));
    try {
      const file = join(dir, 'catalog.json');
      const text = JSON.stringify(catalog);
      await writeFile(file, text, 'utf-8');

      const defineValue = builtInCatalogDefine({ EDDY_CODE_BUILT_IN_CATALOG_FILE: file });
