...
describe('formatSessionLabel', () => {
  it('prepends [imported] when metadata.imported_from_eddy_cli === true', () => {
    const label = formatSessionLabel({
      title: 'Refactor sessions list',
      metadata: { imported_from_eddy_cli: true },
    });
  ...
  it('only triggers on the literal boolean true (not truthy values)', () => {
    const label = formatSessionLabel({
      ...
      metadata: { imported_from_eddy_cli: 'yes' as unknown },
    });
  ...
  it('does not prepend [imported] when flag is false', () => {
    const label = formatSessionLabel({
      ...
      metadata: { imported_from_eddy_cli: false },
    });
