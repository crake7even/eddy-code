...
function makePlan(over: Partial<MigrationPlan> = {}): MigrationPlan {
  return {
    sourceHome: '/x/.eddy-code',
    hasConfig: true,
    hasMcp: true,
    hasUserHistory: true,
    oauthCredentials: ['eddy-code.json'],
    workdirs: [],
    detectedPlugins: [],
...
describe('MigrationScreenComponent — ask phase', () => {
  it('ask1 renders the intro block and three options', () => {
    const c = new MigrationScreenComponent({
      plan: makePlan(),
      sourceHome: '/x/.eddy-code',
      targetHome: '/y/.eddy-code',
      colors: darkColors,
      onComplete: () => {},
    });
    const out = render(c);
    expect(out).toContain('从 eddy-code 迁移');
  ...
  it('ask1 summary does not mention eddy-code login (oauth is not a migrated kind)', async () => {
    // OAuth credentials are deliberately never migrated, so the pre-migration
    // summary must not list "eddy-code login" alongside the real migratable
    ...
    const out = render(c);
    expect(out).not.toContain('eddy-code login');
...
describe('MigrationScreenComponent — progress phase', () => {
  it('renders a step checklist and the session counter when in progress', () => {
    ...
    expect(out).toContain('正在从 eddy-code 迁移');
...
describe('MigrationScreenComponent — result phase', () => {
  it('renders the report summary including plugin notices', () => {
    ...
    expect(out).toContain('2 个 eddy-code 插件');
  });
  ...
  it('surfaces conflict and failure warnings on the result screen', () => {
    ...
    const out = c.render(80).join('\n');
    expect(out).toContain('config.migrated-from-eddy-code.toml');
    expect(out).toContain('mcp.migrated-from-eddy-code.json');
  ...
  it('lists sibling-file contents in the config-fallback warning so the user knows what to merge', () => {
    ...
    // to `config.migrated-from-eddy-code.toml` instead, the result screen must
      ...
      makeReport(
        {},
        {
          config: {
            ...
            droppedHooks: 0,
            siblingContents: {
              providers: ['openai', 'managed:eddy-code'],
              models: ['gpt4'],
              hooks: 3,
            },
          },
  ...
  it('lists kept config settings on the result screen when eddy-code differed', () => {
    const c = new MigrationScreenComponent({
      plan: makePlan(),
      sourceHome: '/x/.eddy-code',
      targetHome: '/y/.eddy-code',
      ...
      makeReport(
        {},
        {
          config: {
            ...
            droppedModels: [],
            droppedKeys: [],
            configConflicts: ['default_model', 'providers.eddy-code'],
            wroteSiblingDueToConflict: false,
            wroteTuiSibling: false,
            migratedHooks: 0,
            droppedHooks: 0,
    ...
    expect(out).toContain('default_model · providers.eddy-code');
  });
