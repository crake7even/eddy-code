...
const SERVER_ENTRY = join(REPO_ROOT, 'plugins/official/eddy-datasource/bin/eddy-datasource.mjs');

describe.skip('eddy-datasource MCP server', () => {
  it('exposes the same two generic tools as the Python plugin', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'eddy-datasource-plugin-'));
    const eddyHome = join(tempDir, 'eddy-home');
    let child: ChildProcessWithoutNullStreams | undefined;

    try {
      await mkdir(join(eddyHome, 'credentials'), { recursive: true });
      await writeFile(
        join(eddyHome, 'credentials', 'eddy-code.json'),
    ...
    try {
      ...
      child = spawn(process.execPath, [SERVER_ENTRY], {
        ...
        env: {
          ...process.env,
          EDDY_CODE_HOME: eddyHome,
          EDDY_DATASOURCE_API_URL: `http://127.0.0.1:${address.port}`,
        },
        stdio: ['pipe', 'pipe', 'pipe'],
      });
      const client = createRpcClient(child);
