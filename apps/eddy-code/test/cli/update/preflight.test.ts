...
import { getSourceInstallDir } from '#/cli/update/source';
import { emptyUpdateCache, type UpdateCache } from '#/cli/update/types';

const mocks = vi.hoisted(() => ({
  readUpdateCache: vi.fn(),
  getDefaultSourceInstallDir: vi.fn(() => '/home/user/.eddy-code'),
  getSourceInstallDir: vi.fn<() => string | null>(() => '/home/user/.eddy-code'),
...
vi.mock('../../../src/cli/update/source', () => ({
  getDefaultSourceInstallDir: mocks.getDefaultSourceInstallDir,
  getSourceInstallDir: mocks.getSourceInstallDir,
}));
...
function expectedInstallCommand(installDir: string): string {
  ...
  return `cd "${installDir}" && git pull origin main && pnpm install && pnpm -r build`;
}
...
describe('runUpdatePreflight', () => {
  ...
  it('continues on first launch with empty cache, still refreshes in background', async () => {
    ...
    expect(getSourceInstallDir).not.toHaveBeenCalled();
  });
  ...
  it('source install: prompts and runs git pull + pnpm install + pnpm -r build', async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.getSourceInstallDir.mockReturnValue('/home/user/.eddy-code');
    ...
    expect(mocks.promptForInstallConfirmation).toHaveBeenCalledWith(
      expect.objectContaining({
        installCommand: expectedInstallCommand('/home/user/.eddy-code'),
    ...
    expect(mocks.spawn).toHaveBeenNthCalledWith(
      ...
      'git',
      ['pull', 'origin', 'main'],
      expect.objectContaining({ cwd: '/home/user/.eddy-code', stdio: 'inherit' }),
    );
    expect(mocks.spawn).toHaveBeenNthCalledWith(
      2,
      'pnpm',
  ...
  it('unsupported: prints manual upgrade command, does not spawn', async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.getSourceInstallDir.mockReturnValue(null);
    const { stdout, options } = captureOutput();
    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
    expect(stdout.join('')).toContain(expectedInstallCommand('/home/user/.eddy-code'));
