    ...
    EddyHarness: class {
      ...
      constructor(...args: unknown[]) {
        ...
        if (mocks.harnessCreatesDeviceIdOnConstruction) {
          mocks.createEddyDeviceId(options?.homeDir ?? '/tmp/eddy-code-test-home');
        }
...
describe('runPrompt', () => {
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('恢复此会话：eddy -r ses_prompt\n');
    expect(mocks.harnessClose).toHaveBeenCalled();
  });

  it('uses the CLI model override when creating a fresh prompt session', async () => {
    await runPrompt(opts({ model: 'eddy-code/k2.5' }), '1.2.3-test', {
  ...
  it('applies the CLI model override to resumed prompt sessions', async () => {
    await runPrompt(opts({ session: 'ses_existing', model: 'eddy-code/k2.5' }), '1.2.3-test', {
      stdout: { write: vi.fn(() => true) },
      stderr: { write: vi.fn(() => true) },
    });

    expect(mocks.harnessResumeSession).toHaveBeenCalledWith({ id: 'ses_existing' });
    expect(mocks.session.setModel).toHaveBeenCalledWith('eddy-code/k2.5');
  });
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"eddy -r ses_prompt","content":"恢复此会话：eddy -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
  ...
  it('throws when no default model is configured', async () => {
    ...
    await expect(
      ...
      '未配置模型。运行 `eddy` 并使用 /login 登录后重试；或在 config.toml 中设置 default_model。',
    );
