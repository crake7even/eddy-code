...
CACHE_VERSION="v1"
FETCHER_VERSION="3"
CACHE_FILE="$ROOT/.git/eddy-code/pnpm-deps-hashes-$CACHE_VERSION.json"
RESTORE_ORIG_HASH=0
...
if nix build --no-link '.#eddy-code-pnpm-deps' >/dev/null 2>&1; then
...
BUILD_OUT="$(nix build --no-link --print-build-logs '.#eddy-code-pnpm-deps' 2>&1 || true)"
...
if ! nix build --no-link '.#eddy-code-pnpm-deps'; then
