...
export class MemoryMemoStore {
   ...
   * Reads `<eddyHomeDir>/sessions/<workDirKey>/memory/entries.jsonl`
   ...
   */
  static async migrateLegacyStores(eddyHomeDir: string): Promise<void> {
    const target = new MemoryMemoStore(eddyHomeDir);
    const markerPath = join(eddyHomeDir, 'memory', MIGRATION_MARKER);
    ...
    const sessionsDir = join(eddyHomeDir, 'sessions');
