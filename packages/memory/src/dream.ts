 ...
 * another dream. Persisted to `<eddyHomeDir>/dream-lock.json`.
 */
export class DreamTracker {
  ...
  constructor(eddyHomeDir: string) {
    this.eddyHomeDir = eddyHomeDir;
    this.lockPath = join(eddyHomeDir, LOCK_FILE);
    this.state = {
   ...
   * - <eddyHomeDir>/.eddy-code/dream-lock.json (buggy double directory)
   * - <eddyHomeDir>/sessions/<sessionKey>/.eddy-code/dream-lock.json (old per-session)
   *
  ...
  private async migrateLegacyLockFiles(): Promise<void> {
    const candidates: string[] = [
      join(this.eddyHomeDir, '.eddy-code', LOCK_FILE),
    ];

    const sessionsDir = join(this.eddyHomeDir, 'sessions');
    try {
      const entries = await readdir(sessionsDir, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory()) {
          candidates.push(join(sessionsDir, entry.name, '.eddy-code', LOCK_FILE));
        }
      }
    } catch {
