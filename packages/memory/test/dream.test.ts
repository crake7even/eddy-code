...
describe('DreamTracker', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'eddy-dream-test-'));
  });
  ...
  it('persists the lock file directly under the eddy home directory', async () => {
  ...
  it('migrates legacy lock files to the new global location', async () => {
    const legacyDir = join(tmpDir, '.eddy-code');
