 ...
 * user-facing install hint. Set `EDDY_SHELL_PATH` to override.
 */
...
async function locateWindowsGitBash(deps: EnvironmentDeps): Promise<string> {
  ...
  const override = deps.env['EDDY_SHELL_PATH']?.trim();
  ...
  throw new JianShellNotFoundError(
    `Git Bash was not found on this Windows host. Install Git for Windows from https://gitforwindows.org/ or set EDDY_SHELL_PATH to a bash.exe. Checked: ${checked.join(', ')}.`,
  );
}
