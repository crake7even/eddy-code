 ...
 *   - Windows resolves Git Bash via `EDDY_SHELL_PATH`, `git.exe` on PATH,
...
describe('detectEnvironment', () => {
  ...
  it('uses EDDY_SHELL_PATH override when set and the file exists', async () => {
    const env = await detectEnvironment(
      stubDeps({
        platform: 'win32',
        env: { EDDY_SHELL_PATH: 'D:\\custom\\bash.exe' },
