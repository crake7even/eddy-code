 ...
 * resumable by — real eddy-core. The migrator writes session buckets named by
 * `computeWorkdirBucket`; eddy-core's session picker (`SessionStore.list`)
 ...
 *     all from `@eddy-code/agent-core/session/store`.
 *   - `Session` (constructor + `resume()` + `agents` map), from
 *     `@eddy-code/agent-core`; `localJian` from `@eddy-code/jian`. After
...
import {
  ...
  encodeWorkDirKey,
  normalizeWorkDir,
} from '@eddy-code/agent-core/session/store';
import { Session, type SDKSessionRPC } from '@eddy-code/agent-core';
import { LocalJian } from '@eddy-code/jian';
...
describe('migrated session loads in real eddy-core', () => {
  it('computeWorkdirBucket matches eddy-core encodeWorkDirKey', () => {
  ...
  it('SessionStore.list() finds a migrated session under the same workDir', async () => {
    ...
    // This exercises eddy-core's bucket lookup end-to-end: list() does
    ...
    expect(migrated?.metadata?.['imported_from_eddy_cli']).toBe(true);
  });
  ...
  it('real eddy-core Session.resume() loads the migrated message history', async () => {
    const result = await migrateOneSession({
      sourceSessionDir: join(FIXTURES, 'tiny-hello-world'),
      oldSessionUuid: 'tiny-resume',
      workdirPath: WORK_DIR,
    ...
    // Drive a real eddy-core resume: `Session.resume()` reads `state.json`
