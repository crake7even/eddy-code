...
const FIXTURE_EDDY = join(FIXTURES, 'multi-workdir', '.eddy-code');
...
beforeEach(async () => {
  ...
  await mkdir(join(FIXTURE_EDDY, 'sessions', PROJ_B_BUCKET, 'uuid-b2'), { recursive: true });
});
afterEach(async () => {
  await rm(targetHome, { recursive: true, force: true });
  await rm(join(FIXTURE_EDDY, 'sessions', PROJ_B_BUCKET, 'uuid-b2'), {
    recursive: true,
    force: true,
  });
});
...
describe('migrateSessionsStep (multi-workdir fixture)', () => {
  ...
  it('counts an already-migrated session as failed when its index entry cannot be ensured', async () => {
    ...
    await migrateSessionsStep({ sourceHome: FIXTURE_EDDY, targetHome });
  ...
  it('counts a content-empty session as skipped-empty (not failed) and still migrates the real one', async () => {
    ...
    // in eddy-code) must be reported as skipped-empty — not failed — without
