...
describe('writeSessionState', () => {
  it('uses custom_title when present', async () => {
    await writeSessionState(dir, {
      ...
      sourcePath: '/Users/me/.eddy-code/sessions/x/y',
    ...
    // session dir — eddy-core reads `wire.jsonl` from here on resume.
    expect(meta.agents.main.homedir).toBe(join(dir, 'agents', 'main'));
    expect(meta.custom.imported_from_eddy_cli).toBe(true);
    expect(meta.custom.eddy_cli_session_id).toBe('old-uuid');
  });
