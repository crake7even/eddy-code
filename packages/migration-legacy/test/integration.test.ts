...
const SOURCE_HOME = join(FIXTURES, 'multi-workdir', '.eddy-code');
const MARKER_PATH = join(SOURCE_HOME, '.migrated-to-eddy-code');
...
describe('runMigration (end-to-end on multi-workdir fixture)', () => {
  ...
  it('does not copy OAuth credentials into the target', async () => {
    ...
    try {
      ...
      await expect(
        readFile(join(tgt, 'credentials', 'eddy-code.json'), 'utf-8'),
      ).rejects.toThrow();
      // The report tells the user to sign in again in eddy-code.
      expect(report.notices.oauthLoginsRequiringRelogin).toContain('eddy-code.json');
    } finally {
      await rm(src, { recursive: true, force: true });
    }
  });
