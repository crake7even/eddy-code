...
[providers."managed:eddy-code".oauth]
storage = "file"
key = "oauth/eddy-code"
`;
...
describe('migrateConfigStep', () => {
  ...
  it('additively merges into a user-modified target config', async () => {
    ...
    const r = await migrateConfigStep({ sourceHome: src, targetHome: tgt });
    expect(r.wroteSiblingDueToConflict).toBe(false);
    // default_thinking is set on both, differently 鈫?target's value is kept
    ...
    expect(cfg).toContain('eddy-code/eddy-for-coding'); // migrated model added
  });
...
`,
    ...
    const r = await migrateConfigStep({ sourceHome: src, targetHome: tgt });
    expect(r.configConflicts).toContain('providers.managed:eddy-code');
    const cfg = await readFile(join(tgt, 'config.toml'), 'utf-8');
    expect(cfg).toContain('https://target.example/v1');
    expect(cfg).not.toContain('https://source.example/v1');
  });

  it('drops top-level keys eddy-code does not support', async () => {
    await writeFile(
      join(src, 'config.toml'),
  ...
  it('falls back to a sibling file when the target config is unparseable', async () => {
    ...
    expect(
      await readFile(join(tgt, 'config.migrated-from-eddy-code.toml'), 'utf-8'),
  ...
  it('a tui.toml-only conflict sets wroteTuiSibling, not wroteSiblingDueToConflict', async () => {
    await writeFile(join(src, 'config.toml'), OLD_CONFIG_TOML);
    // config.toml target is a stub (overwritable) 鈥?only tui.toml conflicts.
    await writeFile(join(tgt, 'config.toml'), DEFAULT_CONFIG_FILE_TEXT);
    ...
    expect(
      await readFile(join(tgt, 'tui.migrated-from-eddy-code.toml'), 'utf-8'),
    ).toContain('theme');
    // original kept
  ...
  it('drops a kept-provider model missing required schema fields', async () => {
    // `bad-model` references the kept `managed:eddy-code` provider but omits
    // `max_context_size`, which eddy-code's ModelAliasSchema requires. Written
    // verbatim it would make getConfig() reject the whole config post-migration.
    const cfg = `[providers."managed:eddy-code"]
type = "eddy"
base_url = "https://api.eddy-code.local/coding/v1"
...
`;
  ...
  it('drops default_model when it points at a model that was not kept', async () => {
    ...
    expect(r.migrated).toBe(true);
    const cfg = await readFile(join(tgt, 'config.toml'), 'utf-8');
    // `ghost-model` has no [models."ghost-model"] entry 鈥?a dangling
...
`;
  ...
  it('drops a supported top-level key whose value the schema rejects', async () => {
    ...
    // 鈥?writing it verbatim would make the next getConfig() reject the file.
...
`,
  ...
  it('drops a legacy theme outside the eddy-code TUI enum', async () => {
  ...
  it('drops a single hook eddy-code\'s schema rejects, keeps the rest', async () => {
    await writeFile(
      join(src, 'config.toml'),
      '[[hooks]]\n' +
        'event = "PreToolUse"\n' +
  ...
  it('reports migratedHooks=0 when target already has the identical hooks (idempotent re-run)', async () => {
    ...
    // "N hooks migrated" again 鈥?`mergeConfig` records no conflict when the
  ...
  it('reports migratedHooks=0 and populates siblingContents when sibling mode kicks in', async () => {
    // The live `config.toml` is unparseable 鈫?migration falls back to writing
    // `config.migrated-from-eddy-code.toml`. Hooks land in the sibling, NOT in
