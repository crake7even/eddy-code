...
describe('migrateSkillsStep', () => {
  it('copies SKILL.md bundles and flat .md skills under ~/.eddy-code/skills/', async () => {
  ...
  it('returns zero counters when source ~/.eddy-code/skills/ is missing', async () => {
    const r = await migrateSkillsStep({ sourceHome: src, targetHome: tgt });
  ...
  it('copies non-skill files at top level too (no filtering)', async () => {
    ...
    // ~/.eddy-code/skills/ is preserved verbatim. The new scanner ignores anything
