...
// eddy-code's tui.toml `theme` enum (mirrors apps/eddy-code TuiThemeSchema).
...
// The config.toml top-level keys eddy-code understands, derived from the live
// EddyConfigSchema so the set tracks eddy-code automatically. `raw` is internal
// — never migrate it. `providers` / `models` / `hooks` are filtered per-entry,
// not via this set.
const SUPPORTED_TOP_LEVEL_KEYS: ReadonlySet<string> = new Set(
  Object.keys(EddyConfigSchema.shape)
...
export interface ConfigStepResult {
  ...
  /** Top-level keys dropped because eddy-code's config schema lacks them. */
  readonly droppedKeys: readonly string[];
  /**
   * Keys/sections the existing target config and the eddy-code config both set
   * to a different value — the target's value was kept.
   */
  readonly configConflicts: readonly string[];
  /** A `config.toml` conflict forced a `config.migrated-from-eddy-code.toml` sibling. */
  readonly wroteSiblingDueToConflict: boolean;
  /** A `tui.toml` conflict forced a `tui.migrated-from-eddy-code.toml` sibling. */
  readonly wroteTuiSibling: boolean;
  /** Count of eddy-code hook entries written into the LIVE target config. */
  readonly migratedHooks: number;
  /** Count of eddy-code hook entries dropped because eddy-code's schema rejects them. */
  readonly droppedHooks: number;
  /**
   * When sibling mode kicks in (`wroteSiblingDueToConflict === true`), the
   * content that landed in `config.migrated-from-eddy-code.toml` instead of
...
/** True when the eddy-code provider entry validates against eddy-code's schema. */
...
/** True when the eddy-code model entry validates against eddy-code's schema. */
...
/**
 * Additively merge the eddy-code config into the existing target config: add
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 1) Providers — keep only those eddy-code's schema accepts.
  ...
  // Provider names the merge resolves to a DIFFERENT entry than the eddy-code
  ...
  // 2) Models — keep only those eddy-code's schema accepts, and not those
  ...
  // 2b) Hooks — keep only entries eddy-code's HookDefSchema accepts. eddy-code
  //     and eddy-code share an identical hook shape, so a valid legacy hook
  //     passes straight through; the per-entry filter only guards against
  //     future schema drift (an event type / field eddy-code does not know).
  ...
  // 4) Build the migrated top-level — only keys eddy-code's schema supports.
  ...
  for (const [k, v] of Object.entries(parsed)) {
    ...
    if (k === 'default_yolo') {
      // eddy-code's `default_yolo` maps to eddy-code's `default_permission_mode`.
    ...
    // written config — one kept from eddy-code, or already in the target being
  ...
  // 4b) Drop any supported top-level key whose VALUE eddy-code's config
  ...
  for (;;) {
    const result = EddyConfigSchema.safeParse(transformTomlData(migratedTop));
  ...
  // In sibling mode the source hooks land in `config.migrated-from-eddy-code.toml`,
