...
// Mirrors eddy-code's `Metadata` pydantic model (metadata.py:43–49).
...
export const OldEddyJsonSchema = z.object({
  work_dirs: z.array(OldWorkDirMetaSchema).default([]),
});

// Mirrors eddy-code's `SessionState` (session_state.py:28–45).
// We use `.passthrough()` because old persisted state may carry extra
// fields from newer eddy-code versions; we only consume a known subset.
...
export type OldEddyJson = z.infer<typeof OldEddyJsonSchema>;
