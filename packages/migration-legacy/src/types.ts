/**
 * Plan describing the contents detected under the source `~/.eddy-code/` directory.
...
/**
 * One workdir bucket (`~/.eddy-code/sessions/<md5>/`) with reverse-looked-up
 * path from `~/.eddy-code/eddy.json`. Buckets with jian != 'local' or no
 * workdir found are excluded from this list (they appear in counters).
 */
export interface WorkDirEntry {
  readonly oldHashDir: string; // absolute path to ~/.eddy-code/sessions/<md5>/
 ...
 * `~/.eddy-code/migration-report.json` and surfaced in the terminal summary.
 */
...
export interface MigrationSummary {
  readonly config: {
    ...
    /**
     * Keys/sections where the existing target config and the eddy-code config
     ...
     * `config.migrated-from-eddy-code.toml` instead of the live `config.toml`.
  ...
  readonly mcp: {
    ...
    /** Source servers dropped because eddy-code's MCP schema rejects them. */
...
export interface MigrationNotices {
  ...
  /**
   * Basenames of eddy-code OAuth logins (`~/.eddy-code/credentials/<name>.json`)
   ...
   * eddy-code instead. Empty when the legacy install had no OAuth login.
   */
