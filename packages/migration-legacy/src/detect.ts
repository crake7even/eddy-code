...
export async function detectMigration(opts: { sourcePath: string }): Promise<MigrationPlan> {
  ...
  // Reverse-lookup workdir from eddy.json
  const workdirMap = new Map<string, WorkdirMeta>();
  try {
    const text = await readFile(sourceEddyJson(src), 'utf-8');
    ...
    // no eddy.json or unparseable 鈥?sessions list will be empty
  }
  ...
  try {
    ...
    // sessions dir missing 鈥?leave workdirs empty
  }
