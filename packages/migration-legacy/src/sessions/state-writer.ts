...
import type { OldSessionState } from '../eddy-cli-schema.js';
...
export async function writeSessionState(sessionDir: string, input: StateWriteInput): Promise<void> {
  ...
  const meta = {
    ...
    agents: {
      main: {
        // eddy-core's `Session.resume()` treats `agents.main.homedir` as the
        // agent's *record directory* 鈥?where it reads `wire.jsonl`. The
    ...
    custom: {
      imported_from_eddy_cli: true,
      eddy_cli_source_path: input.sourcePath,
      eddy_cli_session_id: input.oldSessionUuid,
      eddy_cli_wire_protocol: input.wireProtocolFromOld,
