...
import { OldEddyJsonSchema, OldSessionStateSchema } from '../eddy-cli-schema.js';
import { ensureSessionIndexEntry } from '../session-index.js';
import { sourceEddyJson, sourceSessionsDir } from '../paths.js';
...
export async function migrateSessionsStep(
  ...
  // Stable, deterministic order so progress events emit "newest first" 鈥?the
  ...
  for (const c of candidates) {
    ...
    if (result.outcome === 'migrated') {
      try {
        // `ensureSessionIndexEntry` is idempotent 鈥?if a stale index line for
        ...
        // The session dir is written, but resume-by-id needs the index entry 鈥?
      ...
      // idempotent 鈥?it adds the entry only when absent 鈥?so a rerun
      ...
      try {
        ...
        // The index entry is genuinely missing and could not be added 鈥?the
      ...
      // as skipped, not failed 鈥?`classifySessionDir` usually catches these
...
function resolveBucket(
  ...
  // Pure md5 hex (32 chars) 鈫?look up directly.
...
async function loadWorkdirs(sourceHome: string): Promise<WorkdirMeta[]> {
  try {
    const text = await readFile(sourceEddyJson(sourceHome), 'utf-8');
    const parsed = OldEddyJsonSchema.parse(JSON.parse(text));
