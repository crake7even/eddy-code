...
const PLACEHOLDER_TEXT = '[tool result unavailable — session imported from eddy-code]';

/**
 * Close dangling tool calls so no messages are dropped on resume.
 *
 * eddy-core's context module defers messages while a tool exchange is open
 ...
 * id is satisfied. A eddy-code session interrupted mid-tool-call therefore
