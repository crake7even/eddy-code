...
import { OldSessionStateSchema, type OldSessionState } from '../eddy-cli-schema.js';
...
export async function migrateOneSession(input: MigrateOneInput): Promise<MigrateOneResult> {
  ...
  if (existsSync(targetDir)) {
    ...
    // A dir we wrote ourselves on a previous run 鈥?idempotent re-run.
    if (cls === 'imported') {
      return { outcome: 'already-migrated', targetDir };
    }
    // A real, unrelated eddy-code session occupies the path 鈥?a true conflict.
    if (cls === 'foreign') {
      return { outcome: 'conflict', targetDir };
    }
    // 'debris': `state.json` is absent or corrupt 鈥?a prior migration was
  ...
  try {
    ...
    // missing or corrupt state 鈥?proceed with defaults
  }
  ...
  if (messages.length === 0) {
    ...
    // whose every line failed to parse 鈥?the latter is a real data problem
  ...
  if (wireMtimeS !== null && wireMtimeS !== undefined) {
    ...
    // No recorded `wire_mtime`: fall back to the source `wire.jsonl` mtime 鈥?
    // the SAME signal `migrateSessionsStep`/detection rank recency by 鈥?so
  ...
  // eddy-core's `SessionStore.list()` ranks sessions by the *filesystem*
  // mtimes of `state.json` / `wire.jsonl` / the session dir 鈥?not by the
 ...
 *  - `foreign`:  a real, unrelated eddy-code session occupying the path.
 *  - `debris`:   no `state.json`, or a corrupt/unparseable one 鈥?a prior
...
async function classifyExistingTarget(targetDir: string): Promise<ExistingTarget> {
  ...
  if (
    ...
    custom !== null &&
    (custom as { imported_from_eddy_cli?: unknown }).imported_from_eddy_cli === true
  ) {
    return 'imported';
  }
  return 'foreign';
