...
// Verbatim from packages/eddy-core/src/harness/configs/toml.ts:42
export const DEFAULT_CONFIG_FILE_TEXT =
  '# ~/.eddy-code/config.toml\n' +
  '# Runtime settings for Eddy Code.\n' +
  '# This file starts empty so built-in defaults can apply.\n' +
  '# Login will populate managed Eddy provider and model entries.\n';

const LEGACY_DEFAULT_CONFIG_FILE_TEXT =
  '# ~/.eddy-code/config.toml\n' +
  '# Runtime settings for Eddy Code.\n' +
  '# This file starts empty so built-in defaults can apply.\n' +
  '# Login will populate managed Eddy provider and model entries.\n';

// Verbatim from apps/eddy-code/src/tui/config.ts:renderTuiConfig(DEFAULT_TUI_CONFIG)
export const DEFAULT_TUI_RENDER =
  '# ~/.eddy-code/tui.toml\n' +
  '# Terminal UI preferences for Eddy Code.\n' +
  '# Agent/runtime settings stay in ~/.eddy-code/config.toml.\n' +
  '\n' +
...
export async function isConfigStubOrMissing(configPath: string): Promise<boolean> {
  ...
  return text === DEFAULT_CONFIG_FILE_TEXT || text === LEGACY_DEFAULT_CONFIG_FILE_TEXT;
}
