...
import { EddyCliFetchURLProvider } from '#/tools/providers/eddy-cli-fetch-url';
...
import { EddyCliWebSearchProvider } from '#/tools/providers/eddy-cli-web-search';
...
import type {
  ...
  RegisterToolPayload,
  ReloadPluginsResult,
  RemoveEddyProviderPayload,
  RemovePluginPayload,
  RenameSessionPayload,
  ResumeSessionPayload,
  SessionSummary,
  SetActiveToolsPayload,
  SetEddyConfigPayload,
  SetModelPayload,
  SetModelResult,
...
import { JianShellNotFoundError, LocalJian, type Jian } from '@eddy-code/jian';
import type { WebSearchProvider } from '../tools/builtin';
import type { ToolServices } from '../tools/support/services';

const EDDY_CODE_PROVIDER_NAME = 'managed:eddy-code';
...
export interface EddyCoreOptions {
  readonly homeDir?: string | undefined;
  readonly configPath?: string | undefined;
  readonly runtime?: ToolServices | undefined;
  readonly eddyRequestHeaders?: Record<string, string> | undefined;
  readonly resolveOAuthTokenProvider?: OAuthTokenProviderResolver | undefined;
  readonly skillDirs?: readonly string[];
}

export class EddyCore implements PromisableMethods<CoreAPI> {
  ...
  private config: EddyConfig;
  private readonly userHomeDir: string;
  private readonly eddyRequestHeaders: Record<string, string> | undefined;
  ...
  constructor(
    ...
    options: EddyCoreOptions = {},
  ) {
    this.homeDir = resolveEddyHome(options.homeDir);
    ...
    this.jian = LocalJian.create().catch((error: unknown) => {
      if (error instanceof JianShellNotFoundError) {
        throw new EddyError(ErrorCodes.SHELL_GIT_BASH_NOT_FOUND, error.message);
      }
      throw error;
    });
    this.runtime = options.runtime;
    this.eddyRequestHeaders = options.eddyRequestHeaders;
    this.resolveOAuthTokenProvider = options.resolveOAuthTokenProvider;
    this.skillDirs = options.skillDirs ?? [];
    ensureEddyHome(this.homeDir);
    this.config = loadRuntimeConfig(this.configPath);
    this.sessionStore = new SessionStore(this.homeDir);
    this.plugins = new PluginManager({ eddyHomeDir: this.homeDir });
  ...
  async getEddyConfig(input?: GetEddyConfigPayload): Promise<EddyConfig> {
    if (input?.reload) {
      this.config = loadRuntimeConfig(this.configPath);
    }
    return this.config;
  ...
  async setEddyConfig(input: SetEddyConfigPayload): Promise<EddyConfig> {
  ...
  async removeEddyProvider(input: RemoveEddyProviderPayload): Promise<EddyConfig> {
  ...
  async reloadPlugins(_: EmptyPayload): Promise<ReloadPluginsResult> {
    try {
      ...
      throw new EddyError(
        ...
        { cause: error, details: { eddyHomeDir: this.homeDir } },
      );
    }
  }
  ...
  private assertPluginsLoaded(): void {
    ...
    throw new EddyError(
      ...
      { cause: this.pluginsLoadError, details: { eddyHomeDir: this.homeDir } },
    );
  }

  private async resolveRuntime(config: EddyConfig): Promise<ToolServices> {
    if (this.runtime !== undefined) {
      return this.runtime;
    }
    const runtime = await createRuntimeConfig({
      config,
      eddyRequestHeaders: this.eddyRequestHeaders,
  ...
  private resolveSessionSkillConfig(config: EddyConfig): SessionSkillConfig {
  ...
  private sessionApi(sessionId: string): SessionAPIImpl {
    const session = this.sessions.get(sessionId);
    if (session === undefined) {
      throw new EddyError(ErrorCodes.SESSION_NOT_FOUND, `Session "${sessionId}" was not found`, {
        details: { sessionId },
      });
    }
  ...
  private reloadProviderManager(): EddyConfig {
    return this.config = loadRuntimeConfig(this.configPath);
  }
  ...
  private async refreshSessionRuntimeConfig(
    ...
    for (const model of candidates) {
      try {
        ...
        // config (a stale or migrated model) 鈥?that is the graceful-degrade
...
async function createRuntimeConfig(input: {
  ...
  const fetchService = input.config.services?.eddyCliFetch;

  return {
    urlFetcher:
      fetchService?.baseUrl === undefined
        ? localFetcher
        : new EddyCliFetchURLProvider({
            baseUrl: fetchService.baseUrl,
            localFallback: localFetcher,
            defaultHeaders: input.eddyRequestHeaders,
            cache: fetchCache,
...
function buildWebSearcher(input: {
  ...
  const searchService = input.config.services?.eddyCliSearch;
  ...
  return {
    ...
    tokenProvider:
      service.oauth !== undefined
        ? resolveOAuthTokenProvider?.(EDDY_CODE_PROVIDER_NAME, service.oauth)
        : undefined,
...
function requiredWorkDir(operation: string, value: string): string {
  if (typeof value !== 'string' || value.trim() === '') {
    throw new EddyError(ErrorCodes.REQUEST_WORK_DIR_REQUIRED, `${operation} requires workDir`);
  }
