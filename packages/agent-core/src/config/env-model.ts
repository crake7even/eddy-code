...
export const ENV_MODEL_PROVIDER_KEY = '__eddy_env__';
export const ENV_MODEL_ALIAS_KEY = '__eddy_env_model__';

const ALLOWED_TYPES: readonly ProviderType[] = ['eddy', 'anthropic', 'openai'];

const DEFAULT_BASE_URL: Partial<Record<ProviderType, string>> = {
  openai: 'https://api.openai.com/v1',
  // anthropic: omitted -> let the Anthropic SDK pick its default
  // eddy: omitted -> requires EDDY_MODEL_BASE_URL or explicit config
};

/** Default context window (256K) used when EDDY_MODEL_MAX_CONTEXT_SIZE is unset. */
const DEFAULT_MAX_CONTEXT_SIZE = 262144;

/** Default capabilities when EDDY_MODEL_CAPABILITIES is unset (eddy models support both). */
...
function fail(message: string): never {
  throw new EddyError(ErrorCodes.CONFIG_INVALID, message);
}
...
function parseProviderType(raw: string | undefined): ProviderType {
  if (raw === undefined) return 'eddy';
  const normalized = raw.toLowerCase() as ProviderType;
  if (!ALLOWED_TYPES.includes(normalized)) {
    fail(
      `EDDY_MODEL_PROVIDER_TYPE must be one of ${ALLOWED_TYPES.join(', ')}, got "${raw}".`,
    );
  }
  return normalized;
}
...
// fails fast like the other EDDY_MODEL_* values, instead of silently keeping
...
/**
 * When `EDDY_MODEL_NAME` is set, synthesize one provider + one model alias from
 * the `EDDY_MODEL_*` environment variables and make it the default model.
 ...
 */
export function applyEnvModelConfig(config: EddyConfig, env: Env = process.env): EddyConfig {
  const model = trimmed(env['EDDY_MODEL_NAME']);
  if (model === undefined) return config;

  const apiKey = trimmed(env['EDDY_MODEL_API_KEY']);
  if (apiKey === undefined) {
    fail('EDDY_MODEL_NAME is set but EDDY_MODEL_API_KEY is missing.');
  }

  const maxContextRaw = trimmed(env['EDDY_MODEL_MAX_CONTEXT_SIZE']);
  const maxContextSize =
    maxContextRaw === undefined
      ? DEFAULT_MAX_CONTEXT_SIZE
      : parsePositiveInt(maxContextRaw, 'EDDY_MODEL_MAX_CONTEXT_SIZE');

  const type = parseProviderType(trimmed(env['EDDY_MODEL_PROVIDER_TYPE']));
  const baseUrl = trimmed(env['EDDY_MODEL_BASE_URL']) ?? DEFAULT_BASE_URL[type];
  ...
  const maxOutputRaw = trimmed(env['EDDY_MODEL_MAX_OUTPUT_SIZE']);
  const maxOutputSize =
    maxOutputRaw !== undefined
      ? parsePositiveInt(maxOutputRaw, 'EDDY_MODEL_MAX_OUTPUT_SIZE')
      : undefined;
  const capabilities = parseCapabilities(env['EDDY_MODEL_CAPABILITIES']) ?? DEFAULT_CAPABILITIES;
  const displayName = trimmed(env['EDDY_MODEL_DISPLAY_NAME']);
  const reasoningKey = trimmed(env['EDDY_MODEL_REASONING_KEY']);
  const adaptiveThinking = parseBooleanVar(
    env['EDDY_MODEL_ADAPTIVE_THINKING'],
    'EDDY_MODEL_ADAPTIVE_THINKING',
  );
  ...
  const thinkingMode = trimmed(env['EDDY_MODEL_THINKING_MODE']);
  const thinkingEffort = trimmed(env['EDDY_MODEL_THINKING_EFFORT']);
  ...
  const defaultThinking = parseBooleanVar(
    env['EDDY_MODEL_DEFAULT_THINKING'],
    'EDDY_MODEL_DEFAULT_THINKING',
  );

  const merged: EddyConfig = {
    ...config,
  ...
  // EddyError(CONFIG_INVALID) on violation, matching the explicit checks above.
 ...
 * config.toml 鈥?including via a `getConfig` -> `setConfig` patch round-trip,
 ...
 */
export function stripEnvModelConfig(config: EddyConfig): EddyConfig {
...
function rawDefaultModel(config: EddyConfig): string | undefined {
  const raw = config.raw?.['default_model'];
  return typeof raw === 'string' ? raw : undefined;
}

function rawDefaultThinking(config: EddyConfig): boolean | undefined {
  const raw = config.raw?.['default_thinking'];
  return typeof raw === 'boolean' ? raw : undefined;
}

function rawThinking(config: EddyConfig): ThinkingConfig | undefined {
