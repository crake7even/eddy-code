...
/* ------------------------------------------------------------------ */
/*  Key helpers 鈥?reuse generic snake / camel conversion instead of    */
...
const DEFAULT_CONFIG_FILE_TEXT = `# ~/.eddy-code/config.toml
# Runtime settings for Eddy Code.
# This file starts empty so built-in defaults can apply.
# Login will populate managed Eddy provider and model entries.
`;
...
export function readConfigFile(filePath: string): EddyConfig {
 ...
 * synthesized from `EDDY_MODEL_*` environment variables. Use this everywhere a
...
export function parseConfigString(tomlText: string, filePath = 'config.toml'): EddyConfig {
  ...
  try {
    ...
    throw new EddyError(ErrorCodes.CONFIG_INVALID, `Invalid TOML in ${filePath}: ${error instanceof Error ? error.message : String(error)}`, {
      cause: error,
    });
  }
...
function parseConfigData(data: Record<string, unknown>, filePath: string): EddyConfig {
  const raw = cloneRecord(data);
  const transformed = transformTomlData(data);
  transformed['raw'] = raw;

  try {
    return EddyConfigSchema.parse(transformed);
  } catch (error) {
    throw new EddyError(ErrorCodes.CONFIG_INVALID, `Invalid configuration in ${filePath}: ${formatConfigValidationError(error)}`, {
      cause: error,
    });
  }
}
...
export async function writeConfigFile(filePath: string, config: EddyConfig): Promise<void> {
...
export function configToTomlData(config: EddyConfig): Record<string, unknown> {
  ...
  const scalarFields: (keyof EddyConfig)[] = [
    'defaultProvider',
    'defaultModel',
    'planMode',
    'yolo',
...
function servicesToToml(services: ServicesConfig, rawServices: unknown): Record<string, unknown> {
  ...
  if (services.eddyCliSearch !== undefined) {
    out['eddy_cli_search'] = serviceToToml(services.eddyCliSearch);
  } else {
    delete out['eddy_cli_search'];
  }
  if (services.eddyCliFetch !== undefined) {
    out['eddy_cli_fetch'] = serviceToToml(services.eddyCliFetch);
  } else {
    delete out['eddy_cli_fetch'];
  }
  return out;
}

function serviceToToml(service: EddyCliServiceConfig): Record<string, unknown> {
  const out: Record<string, unknown> = {};
