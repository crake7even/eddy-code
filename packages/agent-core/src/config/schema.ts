...
export const EddyCliServiceConfigSchema = z.object({
  baseUrl: z.string().optional(),
  apiKey: z.string().optional(),
  oauth: OAuthRefSchema.optional(),
  customHeaders: StringRecordSchema.optional(),
...
export type EddyCliServiceConfig = z.infer<typeof EddyCliServiceConfigSchema>;
...
export const ServicesConfigSchema = z.object({
  eddyCliSearch: EddyCliServiceConfigSchema.optional(),
  eddyCliFetch: EddyCliServiceConfigSchema.optional(),
...
export const EddyConfigSchema = z.object({
...
export type EddyConfig = z.infer<typeof EddyConfigSchema>;
...
const EddyCliServiceConfigPatchSchema = EddyCliServiceConfigSchema.partial();
const DuckDuckGoConfigPatchSchema = DuckDuckGoConfigSchema.partial();
const ServicesConfigPatchSchema = z.object({
  eddyCliSearch: EddyCliServiceConfigPatchSchema.optional(),
  eddyCliFetch: EddyCliServiceConfigPatchSchema.optional(),
  duckduckgo: DuckDuckGoConfigPatchSchema.optional(),
});

export const EddyConfigPatchSchema = z
  .object({
...
export type EddyConfigPatch = z.infer<typeof EddyConfigPatchSchema>;

export function getDefaultConfig(): EddyConfig {
  return {
    providers: {},
  };
}

export function validateConfig(config: unknown): EddyConfig {
  try {
    return EddyConfigSchema.parse(config);
  } catch (error) {
    throw new EddyError(ErrorCodes.CONFIG_INVALID, `Invalid configuration: ${formatConfigValidationError(error)}`, {
      cause: error,
    });
  }
}
