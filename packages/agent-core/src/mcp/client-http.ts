...
import {
  buildRequestOptions,
  EDDY_MCP_CLIENT_NAME,
  EDDY_MCP_CLIENT_VERSION,
  toMcpToolDefinition,
  toMcpToolResult,
...
export class HttpMcpClient implements MCPClient {
  ...
  constructor(config: McpServerHttpConfig, options: HttpMcpClientOptions = {}) {
    ...
    this.client = new Client({
      name: options.clientName ?? EDDY_MCP_CLIENT_NAME,
      version: options.clientVersion ?? EDDY_MCP_CLIENT_VERSION,
    });
...
export function buildMcpHttpHeaders(
  ...
  if (config.bearerTokenEnvVar !== undefined) {
    ...
    if (token === undefined || token.length === 0) {
      throw new EddyError(ErrorCodes.CONFIG_INVALID, `MCP HTTP bearer token env var "${config.bearerTokenEnvVar}" is not set or is empty`);
    }
