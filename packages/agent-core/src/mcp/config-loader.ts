...
import { resolveEddyHome } from '#/config/path';
...
export interface McpJsonPaths {
  ...
  /** Parent `.eddy-code/mcp.json` paths, ordered from root to shallowest. */
...
export function resolveMcpJsonPaths(input: ResolveMcpJsonPathsInput): McpJsonPaths {
  ...
  return {
    user: join(resolveEddyHome(input.homeDir), 'mcp.json'),
    project: join(cwd, '.eddy-code', 'mcp.json'),
    parents: findParentMcpJsonPaths(cwd),
  };
}

/** Walk up from `cwd` collecting `.eddy-code/mcp.json` paths (root→shallow). */
function findParentMcpJsonPaths(cwd: string): string[] {
  const paths: string[] = [];
  let dir = dirname(cwd);
  for (let i = 0; i < MAX_PARENT_WALK && dir !== dirname(dir); i++) {
    paths.push(join(dir, '.eddy-code', 'mcp.json'));
    dir = dirname(dir);
  }
 ...
 *   1. `~/.eddy-code/mcp.json` (lowest priority)
 *   2. Parent `.eddy-code/mcp.json` files, root→shallow
 *   3. `<cwd>/.eddy-code/mcp.json` (highest project priority)
 *
...
async function readMcpJson(filePath: string): Promise<Record<string, McpServerConfig>> {
  ...
  try {
    ...
    throw new EddyError(ErrorCodes.CONFIG_INVALID, `Failed to read ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
  ...
  try {
    ...
    throw new EddyError(ErrorCodes.CONFIG_INVALID, `Invalid JSON in ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
  ...
  try {
    ...
    throw new EddyError(ErrorCodes.CONFIG_INVALID, `Invalid MCP server config in ${filePath}: ${describeError(error)}`, {
      cause: error,
    });
  }
}
