 ...
 * `<EDDY_CODE_HOME>/credentials/mcp/` (default
 * `~/.eddy-code/credentials/mcp/`).
 *
...
export function mcpCredentialsDir(eddyHomeDir: string): string {
  return join(eddyHomeDir, 'credentials', 'mcp');
}

export function defaultMcpCredentialsDir(): string {
  return mcpCredentialsDir(join(homedir(), '.eddy-code'));
}
