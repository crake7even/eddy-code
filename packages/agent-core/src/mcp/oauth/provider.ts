 ...
 *    state under `<EDDY_CODE_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.eddy-code`).
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("eddy-code (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `eddy-code (${options.serverName})`;
  }
