...
export interface McpOAuthServiceOptions {
  /** Storage backend; overrides `eddyHomeDir` when supplied. */
  readonly store?: JsonFileStore;
  /** Resolved Eddy home; credentials default to `<eddyHomeDir>/credentials/mcp/`. */
...
export class McpOAuthService {
  ...
  constructor(options: McpOAuthServiceOptions = {}) {
    this.store =
      options.store ??
      new JsonFileStore(
        options.eddyHomeDir === undefined ? undefined : mcpCredentialsDir(options.eddyHomeDir),
      );
