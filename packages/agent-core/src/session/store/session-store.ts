...
export class SessionStore {
  ...
  async fork(input: ForkSessionRecordInput): Promise<SessionSummary> {
    ...
    if (indexed !== undefined) {
      throw new EddyError(ErrorCodes.SESSION_ALREADY_EXISTS, `Session "${input.targetId}" already exists`);
    }
  ...
  async rename(id: string, title: string): Promise<void> {
    ...
    if (normalized.length === 0) {
      throw new EddyError(ErrorCodes.SESSION_TITLE_EMPTY, 'Session title cannot be empty');
    }
    const entry = await this.findExistingSessionEntry(id);
    ...
    try {
      ...
      throw new EddyError(ErrorCodes.SESSION_STATE_NOT_FOUND, `Session "${id}" state.json was not found`, {
        cause: error,
      });
    }
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      throw new EddyError(ErrorCodes.SESSION_STATE_INVALID, `Session "${id}" state.json is invalid`);
    }
  ...
  private async listSessionId(sessionId: string): Promise<readonly SessionSummary[]> {
    try {
      ...
      if (error instanceof EddyError && error.code === ErrorCodes.SESSION_NOT_FOUND) {
        return [];
      }
      throw error;
    }
  ...
  private async findExistingSessionEntry(id: string): Promise<SessionIndexEntry> {
    ...
    throw new EddyError(ErrorCodes.SESSION_NOT_FOUND, `Session "${id}" was not found`, {
...
function normalizeRequiredWorkDir(workDir: string): string {
  if (workDir.trim() === '') {
    throw new EddyError(ErrorCodes.REQUEST_WORK_DIR_REQUIRED, 'listSessions requires workDir');
  }
...
function assertSafeSessionId(id: string): void {
  ...
  throw new EddyError(ErrorCodes.SESSION_ID_INVALID, 'Session id contains unsupported path characters');
}
...
/**
 * Scan `~/.cc-connect/sessions/*.json` and return EddyCode-compatible
 ...
 * skipped — the native EddyCode session takes precedence.
 */
async function listCcConnectSessions(
  ...
  for (const name of entries) {
    ...
    for (const [, ccSession] of Object.entries(parsed.sessions ?? {})) {
      ...
      // Deduplicate: if a native EddyCode session already tracks this
