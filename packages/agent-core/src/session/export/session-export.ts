...
const SESSION_LOG_REL = 'logs/eddy-code.log';
const GLOBAL_LOG_REL = 'logs/global/eddy-code.log';
...
export async function exportSessionDirectory(input: {
  ...
  if (sessionFiles.length === 0) {
    throw new EddyError(ErrorCodes.SESSION_EXPORT_NOT_FOUND, `Session "${input.summary.id}" has no exportable directory at "${sessionDir}"`, {
