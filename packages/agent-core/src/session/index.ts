...
import type { EddyConfig, SDKSessionRPC } from '#/rpc';
...
const BACKGROUND_KEEP_ALIVE_ON_EXIT_ENV = 'EDDY_CODE_BACKGROUND_KEEP_ALIVE_ON_EXIT';
...
export class Session {
  ...
  constructor(public readonly options: SessionOptions) {
    ...
    this.mcp = new McpConnectionManager({
      oauthService: new McpOAuthService({ eddyHomeDir: options.eddyHomeDir }),
      log: this.log,
    });
  ...
  private instantiateAgent(
    ...
    return new Agent({
      ...
      homedir,
      eddyHomeDir: this.options.eddyHomeDir,
      skills: this.skills,
  ...
  private ensureResumeAgentInstantiated(
    ...
    if (meta === undefined) {
      throw new EddyError(ErrorCodes.SESSION_STATE_INVALID, `Session agent "${id}" is missing`);
    }
  ...
  private requireMainAgent(): Agent {
    ...
    if (agent === undefined) {
      throw new EddyError(ErrorCodes.AGENT_NOT_FOUND, 'Main agent was not found');
    }
    return agent;
  }
