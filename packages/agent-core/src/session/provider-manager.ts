...
import { ErrorCodes, isEddyError, EddyError } from '../errors';
...
interface ProviderManagerOptions {
  readonly config: EddyConfig | (() => EddyConfig);
...
export class ProviderManager implements ModelProvider {
  ...
  resolveProviderConfig(model: string): ResolvedRuntimeProvider {
    ...
    const provider = toLtodProviderConfig(
      providerConfig,
      alias.model,
      this.options.eddyRequestHeaders,
      alias.maxOutputSize,
      alias.reasoningKey,
  ...
  resolveAuth(
    ...
    if (providerApiKey(providerConfig) !== undefined) {
      ...
      throw new EddyError(
        ErrorCodes.CONFIG_INVALID,
        `Provider "${providerName}" has both apiKey and oauth set in config.toml 鈥?they are mutually exclusive. Remove one.`,
      );
    }

    const loginRequired = (cause?: unknown): EddyError =>
      new EddyError(
    ...
    const fetchAuth = async (force: boolean): Promise<ProviderRequestAuth> => {
      ...
      try {
        ...
        if (!isEddyError(error) || error.code !== ErrorCodes.AUTH_LOGIN_REQUIRED) {
...
function toLtodProviderConfig(
  ...
  model: string,
  eddyRequestHeaders: Record<string, string> | undefined,
  ...
  switch (provider.type) {
    ...
    case 'eddy':
      return {
        type: 'eddy',
        model,
        baseUrl: providerValue(provider.baseUrl, provider.env, 'EDDY_BASE_URL'),
        apiKey: providerApiKey(provider),
        generationKwargs: { prompt_cache_key: promptCacheKey },
        ...defaultHeadersField({ ...eddyRequestHeaders, ...provider.customHeaders }),
      };
    case 'google-genai':
      return {
        type: 'google-genai',
...
// there are no headers 鈥?callers and tests rely on `'defaultHeaders' in provider`.
...
function providerApiKey(provider: ProviderConfig): string | undefined {
  switch (provider.type) {
    ...
    case 'eddy':
      return providerValue(provider.apiKey, provider.env, 'EDDY_API_KEY');
    case 'google-genai':
