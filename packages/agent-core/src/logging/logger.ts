...
const ROOT_SYMBOL = Symbol.for('eddy.logger.root');
const SESSION_LOG_ID = Symbol('eddy.logger.sessionLogId');
...
class RootLoggerImpl implements RootLogger {
  ...
  attachSession(input: SessionAttachInput): SessionLogHandle {
    ...
    const sink = new RotatingFileSink({
      path: join(input.sessionDir, 'logs', 'eddy-code.log'),
 ...
 *
 *   import { log } from 'eddy-code-sdk';
 *   log.info('eddy-code starting', { version });
 *
 ...
 * importing `log` at module load (before `EddyHarness` configures the root)
 * is safe 鈥?calls during the pre-configure window are silent noops.
 */
...
export function resolveGlobalLogPath(homeDir: string): string {
  return join(homeDir, 'logs', 'eddy-code.log');
}
