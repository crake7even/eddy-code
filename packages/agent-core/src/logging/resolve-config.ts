 ...
 *
 *   EDDY_LOG_LEVEL=debug
 *   EDDY_LOG_GLOBAL_MAX_BYTES=... EDDY_LOG_GLOBAL_FILES=...
 *   EDDY_LOG_SESSION_MAX_BYTES=... EDDY_LOG_SESSION_FILES=...
 */
export function resolveLoggingConfig(input: ResolveLoggingInput): LoggingConfig {
  const env = input.env ?? process.env;
  return {
    level: parseLevel(env['EDDY_LOG_LEVEL']) ?? DEFAULT_LOG_LEVEL,
    globalLogPath: resolveGlobalLogPath(input.homeDir),
    globalMaxBytes: parsePositiveInt(env['EDDY_LOG_GLOBAL_MAX_BYTES']) ?? DEFAULT_GLOBAL_MAX_BYTES,
    globalFiles: parsePositiveInt(env['EDDY_LOG_GLOBAL_FILES']) ?? DEFAULT_GLOBAL_FILES,
    sessionMaxBytes:
      parsePositiveInt(env['EDDY_LOG_SESSION_MAX_BYTES']) ?? DEFAULT_SESSION_MAX_BYTES,
    sessionFiles: parsePositiveInt(env['EDDY_LOG_SESSION_FILES']) ?? DEFAULT_SESSION_FILES,
  };
}
