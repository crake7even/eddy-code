 ...
 *     duplicate idle flag 鈥?the turn machinery already knows;
 ...
 *     (via {@link addTask} / {@link removeTasks}) so that `eddy resume`
 ...
 *     entry point is not wired by the framework 鈥?it stays exposed so
 ...
 * fire `origin`. One-shot tasks never carry the stale flag 鈥?they are
 * one-time, "we always fire at most once" by construction. Disabled by
 * `EDDY_CRON_NO_STALE=1` (bench / acceptance tests).
 *
...
export interface CronManagerOptions {
   ...
   * `resolveClockSources(process.env.EDDY_CRON_CLOCK)` so production
   * picks up `EDDY_CRON_CLOCK=file:...` automatically.
   ...
   * (1000ms unless `EDDY_CRON_MANUAL_TICK=1`, which forces `null` here
   * so the auto-tick `setInterval` is never installed). `null` or `0`
   * means "no automatic timer 鈥?caller drives `tick()` manually".
   */
...
export class CronManager {
   ...
   * Used to keep `start()` / `stop()` idempotent and 鈥?more importantly
   * for P1.8 鈥?to gate SIGUSR1 binding so we don't accumulate handlers
   ...
   * `sessionDir` was supplied 鈥?the manager then behaves as pure
   * in-memory, matching pre-persistence semantics. When defined,
   * `addTask` / `removeTasks` schedule fire-and-forget writes so a
   * later `eddy resume` can reload via {@link loadFromDisk}.
   */
  private readonly persistStore: PerIdJsonStore<CronTask> | undefined;

  /**
   * Per-id serializer for persistence writes. Prevents a fast
   * `add` 鈫?`remove` sequence on the same id from racing each other on
   * the rename 鈥?the rm must observe the prior write's renamed file.
  ...
  constructor(agent: Agent, opts: CronManagerOptions = {}) {
    ...
    this.clocks =
      opts.clocks ??
      resolveClockSources(process.env['EDDY_CRON_CLOCK']) ??
      SYSTEM_CLOCKS;
    this.persistStore =
    ...
    this.scheduler = createCronScheduler({
      ...
      isKilled: () => process.env['EDDY_DISABLE_CRON'] === '1',
      ...
      // P1.8: `EDDY_CRON_MANUAL_TICK=1` forces the scheduler into
      ...
      pollIntervalMs:
        process.env['EDDY_CRON_MANUAL_TICK'] === '1'
          ? null
          : opts.pollIntervalMs,
    });
   ...
   * concurrent writes on the same id (e.g. add 鈫?stale auto-expire) so
   * the rm cannot race the rename.
   *
   * Persistence failures are logged via `agent.log.warn` and swallowed
   * 鈥?a flaky disk drops cross-resume durability but must not crash
   * the agent loop.
   */
   ...
   * `SessionCronStore.remove`'s contract 鈥?callers (CronDelete /
   ...
   * so a `eddy resume` does not coalesce-replay an already-delivered
   ...
   * update the in-memory record 鈥?same-session stale checks read off
   ...
   * stale auto-expire) would otherwise race on the rename 鈥?atomicWrite
  ...
  async flushPersist(): Promise<void> {
    // Snapshot the chain promises rather than the map itself 鈥?the
   ...
   * and signal-handler-safe 鈥?multiple vitest files exercising the
   ...
   * CronList renders the same instant the scheduler will fire 鈥?even
   ...
   * Stale judgment.
   *
   *   - `EDDY_CRON_NO_STALE=1` short-circuits to false (bench).
   *   - One-shot tasks (`recurring === false`) are never stale 鈥?they
  ...
  isStale(task: CronTask): boolean {
    if (process.env['EDDY_CRON_NO_STALE'] === '1') return false;
   ...
   * and stops re-firing the task. One-shots are not affected 鈥?they
  ...
  private handleFire(
    ...
    // 7-day auto-expire 鈥?the recurring branch of CronCreate's tool
   ...
   * 鈥?e.g. for one-shots whose fire times all landed during a long
   * outage 鈥?should not require an API change here.
   *
   ...
   * trivial renderer.
   *
   * `count: 0` is a no-op 鈥?the scheduler-side missed-task detector
   ...
   *
   * Gated on `EDDY_CRON_MANUAL_TICK=1` for two reasons:
   *
   *   1. SIGUSR1 only makes sense when auto-tick is off. When the 1s
   *      interval is running, it already advances the scheduler 鈥?a
   ...
   *
   * Idempotent 鈥?repeated calls keep the same listener registered once,
   * so `start() 鈫?start()` does not stack handlers.
   *
   ...
   * Set `EDDY_CRON_DEBUG=1` to surface the swallowed error to stderr 鈥?
   * mirrors `scheduler.ts`'s debugLog pattern so bench debugging can
   * see a bad tick.
   */
  private bindSigusr1(): void {
    if (process.platform === 'win32') return;
    if (process.env['EDDY_CRON_MANUAL_TICK'] !== '1') return;
    if (this.sigusr1Handler !== null) return;
    const handler: NodeJS.SignalsListener = () => {
      try {
        this.tick();
      } catch (error) {
        if (process.env['EDDY_CRON_DEBUG'] === '1') {
   ...
   * vitest files don't leak signal handlers across the shared process 鈥?
