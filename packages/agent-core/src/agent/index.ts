...
import { ErrorCodes, EddyError, makeErrorPayload } from '#/errors';
import { log } from '#/logging/logger';
import type { Logger } from '#/logging/types';
import type { AgentAPI, AgentEvent, EddyConfig, SDKAgentRPC, UsageStatus } from '#/rpc';
import {
  generate,
  type ChatProvider,
  type Message,
...
import { DreamTracker, EXIT_EXTRACTION_SYSTEM_PROMPT, MemoryMemoStore, buildExitExtractionPrompt, createFastEmbedEngine, parseMemoryMemos } from '@eddy-code/memory';
...
import type { Jian } from '@eddy-code/jian';
...
const SIDE_QUESTION_SYSTEM =
  'You are a helpful coding assistant answering a quick side question. The user is in the middle of a coding session and needs a fast, concise answer. Keep your response short and focused 鈥?this is a side question, not the main task.';

export interface AgentOptions {
  readonly jian: Jian;
  readonly config?: EddyConfig;
  readonly homedir?: string;
  readonly eddyHomeDir?: string;
...
export class Agent {
  readonly type: AgentType;
  readonly jian: Jian;
  readonly eddyConfig?: EddyConfig;
  readonly homedir?: string;
  readonly eddyHomeDir?: string;
  readonly rpc?: Partial<SDKAgentRPC>;
  readonly toolServices?: ToolServices;
  ...
  constructor(options: AgentOptions) {
    ...
    this.eddyConfig = options.config;
    this.homedir = options.homedir;
    this.eddyHomeDir = options.eddyHomeDir;
    ...
    const eddyHomeDir = options.eddyHomeDir;
    this.memoStore = eddyHomeDir
      ? new MemoryMemoStore(eddyHomeDir)
      : undefined;
    if (this.memoStore !== undefined && eddyHomeDir !== undefined) {
      void this.memoStore.init();
      void MemoryMemoStore.migrateLegacyStores(eddyHomeDir);
      // Attach embedding engine for semantic search. Best-effort 鈥?gracefully
      // degrades to keyword-only if fastembed fails to load.
      try {
        this.memoStore.setEmbeddingEngine(createFastEmbedEngine());
      } catch {
        // fastembed not available 鈥?keyword search still works.
      }
    }
    this.sessionMemory = new SessionMemory(this);
    this.workingSet = new WorkingSet();
    this.dreamTracker = new DreamTracker(eddyHomeDir ?? '');
  ...
  get llm(): LtodLLM {
    ...
    const loopControl = this.eddyConfig?.loopControl;
  ...
  get rpcMethods(): PromisableMethods<AgentAPI> {
    return {
      ...
      activateSkill: (payload) => {
        if (this.skills === null) {
          throw new EddyError(ErrorCodes.SKILL_NOT_FOUND, `Skill "${payload.name}" was not found`);
        }
  ...
  async getSessionTitle(): Promise<string | undefined> {
    ...
    try {
      ...
      // ignore 鈥?state.json may not exist
    }
    return undefined;
  }
 ...
 * Keep this interface simple and avoid dynamic keys 鈥?the shape is
