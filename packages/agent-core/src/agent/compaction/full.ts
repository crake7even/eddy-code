...
import { parseMemoryMemos } from '@eddy-code/memory';
...
const COMPACTION_SYSTEM_PROMPT =
  ...
  'Pay special attention to the Memory Memo Extraction section 鈥?' +
...
export class FullCompaction {
    ...
    this.strategy =
      ...
      new DefaultCompactionStrategy(
        () => agent.config.modelCapabilities.max_context_tokens,
        {
          ...
          reservedContextSize:
            agent.eddyConfig?.loopControl?.reservedContextSize ??
            DEFAULT_COMPACTION_CONFIG.reservedContextSize,
          triggerRatio:
            agent.eddyConfig?.loopControl?.compactionTriggerRatio ??
  ...
  begin(data: Readonly<CompactionBeginData>): void {
    ...
    if (compactedCount === 0) {
      throw new EddyError(ErrorCodes.COMPACTION_UNABLE, 'No prefix that can be compacted in current history.');
    }
  ...
  async beforeStep(signal: AbortSignal): Promise<void> {
    ...
    // Stage 2a: Proactive check 鈥?will the NEXT step overflow?
  ...
  private beginAutoCompaction(throwOnLimit: boolean = true): boolean {
    ...
    if (this.consecutiveCompactionFailures >= MAX_CONSECUTIVE_FAILURES) {
      // Circuit breaker open 鈥?auto compaction is disabled for this turn.
    ...
    if (this.compactionCountInTurn >= maxCompactions) {
      if (throwOnLimit) {
        throw new EddyError(ErrorCodes.CONTEXT_OVERFLOW, `Compaction limit exceeded (${String(maxCompactions)})`, {
  ...
  private async block(signal: AbortSignal): Promise<void> {
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (this.compacting === active) {
        this.markCanceled(
          'Compaction timed out after 60 seconds; canceled. Use /compact to retry.',
        );
      }
    }, BLOCK_TIMEOUT_MS);
    ...
    try {
      ...
      for (let i = 0; i < originalHistory.length; i++) {
        if (newHistory[i] !== originalHistory[i]) {
          this.markCanceled('Context changed during compaction; canceled.');
          return undefined;
        }
      }
      ...
      // Compaction succeeded 鈥?reset circuit breaker
      this.consecutiveCompactionFailures = 0;
    } catch (error) {
      if (!isAbortError(error)) {
        ...
        if (this.consecutiveCompactionFailures >= MAX_CONSECUTIVE_FAILURES) {
          this.agent.emitEvent({
            type: 'warning',
            message: `Compaction failed ${String(this.consecutiveCompactionFailures)} times in a row; automatic compaction is paused for this turn. Use /compact to retry.`,
            code: 'compaction_circuit_open',
          });
        }

        this.agent.log.error('compaction failed', {
          code: isEddyError(error) ? error.code : undefined,
          error,
        });
        this.markCanceled();
        if (!blockedByTurn) {
          const payload =
            isEddyError(error) && error.code === ErrorCodes.AUTH_LOGIN_REQUIRED
              ? toEddyErrorPayload(error)
        ...
        if (blockedByTurn) {
          if (isEddyError(error) && error.code === ErrorCodes.AUTH_LOGIN_REQUIRED) throw error;
          throw new EddyError(ErrorCodes.COMPACTION_FAILED, String(error), { cause: error });
        }
      }
    }
  }
