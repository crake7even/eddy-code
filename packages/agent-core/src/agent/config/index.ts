...
export class ConfigState {
  ...
  update(changed: AgentConfigUpdateData): void {
    ...
    if (changed.thinkingLevel !== undefined) {
      this._thinkingLevel = resolveThinkingEffort(
        ...
        this.agent.eddyConfig?.thinking,
      );
    }
  ...
  get providerConfig(): ProviderConfig {
    ...
    if (provider === undefined) {
      throw new EddyError(ErrorCodes.MODEL_NOT_CONFIGURED, 'Provider not set');
    }
    return provider;
  }
  ...
  get model(): string {
    if (this._modelAlias === undefined) {
      throw new EddyError(ErrorCodes.MODEL_NOT_CONFIGURED, 'Model not set');
    }
