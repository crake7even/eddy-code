...
const GOAL_CONTINUATION_PROMPT = [
  ...
  'with `blocked`. Otherwise keep going 鈥?use the existing conversation context and your tools,',
...
export class TurnFlow {
   ...
   * Goal-agnostic 鈥?the driver layers goal semantics on top. Never throws;
  ...
  private async runTurn(turnId: number, signal: AbortSignal): Promise<LoopTurnStopReason> {
    ...
    while (true) {
      ...
      const loopControl = this.agent.eddyConfig?.loopControl;
      try {
        const result = await runTurn({
          ...
          hooks: {
            ...
            shouldContinueAfterStop: async ({ signal }) => {
              ...
              // Hard convergence gate: never stop while there is unverified
              // work, a blocking tool failure, an empty assistant step, or a
              // missing TodoList update for an active goal. Exploratory failures
              // (e.g. probing for tsc, ls, which) do not block once the turn has
              // produced a successful resolution. A passing verification for the
              // current turn also resolves unverified-change concerns.
              ...
              if (this.convergenceInjections < this.MAX_CONVERGENCE_INJECTIONS) {
                ...
                if (unverified.length > 0 && !hasPassedVerificationThisTurn) {
                  const suggestions = this.agent.workingSet.suggestVerificationCommands(
                    this.agent.config.cwd,
                  );
                  reasons.push(
                    `You have unverified changes in: ${unverified.join(', ')}.\n` +
...
[system: verification skipped 鈥?identical successful run within the last ${Math.round(
            ...
            finalizeToolResult: async (ctx) => {
              ...
              void this.agent.hooks?.fireAndForgetTrigger(event, {
                ...
                inputData: {
                  ...
                  error: isError === true ? toEddyErrorPayload(toolOutputText(output)) : undefined,
        ...
        if (
          ...
          (isEddyError(error) && error.code === ErrorCodes.CONTEXT_OVERFLOW)
        ) {
        ...
        if (isMaxStepsExceededError(error)) {
          this.agent.log.warn('turn hit max steps', {
            ...
            limit: isEddyError(error) ? error.details?.['maxSteps'] : undefined,
          });
        } else {
...
const TRIVIAL_COMPLETION_RE =
  /^\s*(done|ok|okay|瀹屾垚|濂戒簡|ok\.?|done\.?|completed\.?|finished\.?|tests?\s+passed\.?|passed\.?|it\s+works\.?|looks\s+good\.?|fixed\.?|resolved\.?|verified\.?|all\s+good\.?|涓€鍒囨甯竆.?|宸插畬鎴怽.?)\s*$/iu;
...
const LLM_NOT_SET_MESSAGE =
  'No model configured. Run `eddy config` or use `/model` to set a default model.';

function summarizeTurnError(error: unknown, turnId: number): EddyErrorPayload {
  const payload = toEddyErrorPayload(error);
...
function summarizeToolArgs(args: unknown): string {
  ...
  // Common tool arg patterns 鈥?try each in priority order
