...
export class BackgroundManager extends BackgroundProcessManager {
  ...
  constructor(public readonly agent: Agent) {
    super({
      maxRunningTasks: agent.eddyConfig?.background?.maxRunningTasks,
 ...
 * 鈥?which id to pass, how to distinguish it from the look-alike `source_id`,
...
function buildBackgroundTaskNotificationBody(
  ...
  const recovery = [
    ...
    `Use agent_id ("${agentId}"), NOT source_id / task_id ("${info.taskId}") 鈥?the two look alike but only agent_id is accepted by the resume parameter.`,
