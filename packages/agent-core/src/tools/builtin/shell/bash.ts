...
import type { Jian, JianProcess } from '@eddy-code/jian';
import { z } from 'zod';
...
function rejectDangerousCommand(
  ...
  return {
    isError: true,
    output:
      `Eddy Code self-protection blocked this command.\n\n` +
      `The command matches a dangerous pattern (${pattern}) that could kill Eddy Code's own process.\n\n` +
      `Instead: ${hint}`,
  };
}
...
function validateCommand(command: string, isWindows: boolean): ExecutableToolResult | null {
  ...
  // killall / pkill targeting node or eddy (POSIX + Git Bash)
  if (/\b(killall|pkill)\b.*\b(node|eddy)/i.test(cmd)) {
    return rejectDangerousCommand(
      'killall/pkill node',
  ...
  if (isWindows) {
    // tasklist + grep node/eddy + taskkill pipeline
    if (
      /\btasklist\b/.test(cmd) &&
      /\bgrep\b.*\b(node|eddy)/i.test(cmd) &&
    ...
    // ps + grep node/eddy + xargs kill pipeline (POSIX)
    if (
      /\bps\b/.test(cmd) &&
      /\bgrep\b.*\b(node|eddy)/i.test(cmd) &&
    ...
    // pgrep node/eddy + xargs kill
    if (/\bpgrep\b.*\b(node|eddy)/i.test(cmd) && /\bxargs\s+kill\b/.test(cmd)) {
      return rejectDangerousCommand(
...
function buildSelfProtectionPreamble(isWindows: boolean): string {
  if (isWindows) {
    ...
    return (
      `_EDDY_CHECK(){ for _a in "$@";do [ "$_a" = "$EDDY_PID" ]&&{ ` +
      `echo "Eddy Code self-protection: refusing to kill itself (pid $EDDY_PID). Use a specific non-Eddy Code PID.">&2;return 1;` +
      `};done;return 0;};` +
      `kill(){ _EDDY_CHECK "$@"||return 1;command kill "$@";};` +
      `pkill(){ echo "Eddy Code self-protection: pkill blocked. Use kill <pid>.">&2;return 1;};` +
      `taskkill(){ _EDDY_CHECK "$@"||return 1;command taskkill "$@";};` +
      `tskill(){ _EDDY_CHECK "$@"||return 1;command tskill "$@";};`
    );
  }
  // POSIX: shadow kill, pkill, killall; also guard process-group kill (-pid)
  return (
    `_EDDY_CHECK(){ for _a in "$@";do [ "$_a" = "$EDDY_PID" ]||[ "$_a" = "-$EDDY_PID" ]&&{ ` +
    `echo "Eddy Code self-protection: refusing to kill itself (pid $EDDY_PID). Use a specific non-Eddy Code PID.">&2;return 1;` +
    `};done;return 0;};` +
    `kill(){ _EDDY_CHECK "$@"||return 1;command kill "$@";};` +
    `pkill(){ echo "Eddy Code self-protection: pkill blocked. Use kill <pid>.">&2;return 1;};` +
    `killall(){ echo "Eddy Code self-protection: killall blocked. Use kill <pid>.">&2;return 1;};`
  );
}
