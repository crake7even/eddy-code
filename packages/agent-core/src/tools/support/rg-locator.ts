 ...
 *   1. System PATH (`which rg`) 鈥?fastest, respects developer setup
 *   2. Bundled vendor binary (hook; not wired yet 鈥?`getVendorRgPath` is a stub)
 *   3. `<EDDY_CODE_HOME>/bin/rg` 鈥?persistent cache for this app.
 *   4. CDN download to <EDDY_CODE_HOME>/bin/ 鈥?one-off bootstrap
 *
...
function getShareDir(): string {
  const override = process.env['EDDY_CODE_HOME'];
  if (override !== undefined && override !== '') return override;
  return join(homedir(), '.eddy-code');
}
...
/** @internal for tests 鈥?rust-style `<arch>-<vendor>-<os>` target triple. */
...
async function downloadAndInstallRg(shareDir: string): Promise<string> {
  ...
  const tmp = await mkdtemp(join(tmpdir(), 'eddy-rg-'));
  try {
    ...
    if (isWindows) {
      ...
      // and known from provider releases; zip branch uses basename matching as
...
/** @internal for tests 鈥?fail closed before extracting downloaded bytes. */
 ...
 * changed" sentinel when the archive holds no matching entry 鈥?same
  ...
  await new Promise<void>((resolve, reject) => {
    yauzlFromBuffer(buf, { lazyEntries: true }, (openErr, zipfile) => {
      ...
      const onEntry = (entry: Entry): void => {
        // Match on basename (not full path) 鈥?keeps the matcher robust
