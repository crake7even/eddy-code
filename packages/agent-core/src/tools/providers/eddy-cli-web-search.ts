/**
 * EddyCliWebSearchProvider — host-side `WebSearchProvider`.
 *
...
export interface EddyCliWebSearchProviderOptions {
...
interface EddyCliSearchResponse {
  search_results?: EddyCliSearchResult[];
}

export class EddyCliWebSearchProvider implements WebSearchProvider {
  ...
  constructor(options: EddyCliWebSearchProviderOptions) {
  ...
  async search(
    ...
    if (response.status === 401) {
      ...
      throw new Error(
        `EddyCli search request failed: HTTP 401 (auth/unauthorized). ${detail}`.trim(),
      );
    }
    ...
    if (response.status !== 200) {
      const detail = await safeReadText(response);
      throw new Error(
        `EddyCli search request failed: HTTP ${String(response.status)}. ${detail}`.trim(),
      );
    }

    const json = (await response.json()) as EddyCliSearchResponse;
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error(
      'EddyCli search service is not configured: missing API key or token provider.',
    );
  }
}
