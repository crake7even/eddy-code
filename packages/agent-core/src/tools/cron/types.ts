 ...
 *   - `cron` 鈥?5-field expression, evaluated in local time.
 *   - `createdAt` 鈥?wall-clock epoch ms at original scheduling. NOT updated
 ...
 *   - `recurring` 鈥?undefined / true means "fire repeatedly until deleted
 *     or auto-expired"; false means "fire once then auto-delete".
 *   - `lastFiredAt` 鈥?wall-clock epoch ms of the last ideal occurrence
 *     whose jittered delivery has actually completed. Persisted so a
 *     `eddy resume` does not replay already-delivered recurring fires:
