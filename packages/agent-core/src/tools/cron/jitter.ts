 ...
 * am") then every CLI fires at the same instant and the provider API
 ...
 * stays intact when {@link EDDY_CRON_NO_JITTER} is set.
 *
 * Two flavours:
 *
...
function jitterDisabledByEnv(): boolean {
  return process.env['EDDY_CRON_NO_JITTER'] === '1';
}
 ...
 * expression — should have been rejected provider) we fall back to a
