...
- `id` 鈥?the 8-hex task id. Pass this to `CronDelete` to remove the
  task, or quote it in user-facing messages when asking for
  confirmation.
- `cron` 鈥?the verbatim 5-field cron expression as scheduled.
- `humanSchedule` 鈥?plain-English rendering (e.g. `every 5 minutes`).
- `prompt` 鈥?the scheduled prompt text, JSON-encoded so embedded
  newlines stay on one line. Truncated to 200 UTF-8 bytes with
  `鈥?truncated)` if longer. Use this to recall what a task is for
  after a context compaction, and as the source for the
  `CronCreate` refresh ritual.
- `nextFireAt` 鈥?ISO timestamp of the next fire **after jitter has
...
- `recurring` 鈥?`true` for cadenced jobs, `false` for one-shots.
- `ageDays` 鈥?`(now - createdAt) / day`, two decimal places. Useful
  when deciding whether a long-running cron is still relevant.
- `stale` 鈥?`true` when a recurring task is older than 7 days. The
  ...
  stale 鈥?they fire at most once by construction.
  `EDDY_CRON_NO_STALE=1` disables the judgment entirely (bench /
  acceptance runs).
...
- The empty case returns `cron_jobs: 0\nNo cron jobs scheduled.`. Cron
  tasks survive a `eddy resume` of the same session but do not bleed
  into new sessions.
