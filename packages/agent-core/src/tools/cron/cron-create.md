...
For "remind me at X" or "at <time>, do Y" requests 鈥?fire once then auto-delete.
Pin minute/hour/day-of-month/month to specific values:
  "remind me at 2:30pm today to check the deploy" 鈫?cron: "30 14 <today_dom> <today_month> *", recurring: false
  "tomorrow morning, run the smoke test" 鈫?cron: "57 8 <tomorrow_dom> <tomorrow_month> *", recurring: false
...
Every user who asks for "9am" gets `0 9`, and every user who asks for "hourly" gets `0 *` 鈥?which means requests from across the planet land on the API at the same instant. When the user's request is approximate, pick a minute that is NOT 0 or 30:
  "every morning around 9" 鈫?"57 8 * * *" or "3 9 * * *" (not "0 9 * * *")
  "hourly" 鈫?"7 * * * *" (not "0 * * * *")
  "in an hour or so, remind me to..." 鈫?pick whatever minute you land on, don't round

Only use minute 0 or 30 when the user names that exact time and clearly means it ("at 9:00 sharp", "at half past", coordinating with a meeting). When in doubt, nudge a few minutes early or late 鈥?the user will not notice, and the fleet will.
...
again with the same `cron` and `prompt` 鈥?that resets `createdAt` and
starts a fresh 7-day window. One-shot tasks are never marked stale.

Bench / acceptance runs can set `EDDY_CRON_NO_STALE=1` to disable the
judgment entirely.

## Jitter behavior

Anti-herd jitter is applied deterministically per task id:
  - Recurring: ideal fire time is shifted **forward** by an offset 鈮?min(10% of the cron period, 15 minutes). A `*/5 * * * *` task can drift up to 30s; a `0 9 * * *` task can drift up to 15 minutes.
  - One-shot: only when the ideal fire lands on `:00` or `:30` of the hour, the fire is pulled **earlier** by 鈮?90 seconds. Other minutes pass through unchanged.

Bench / acceptance tests can set `EDDY_CRON_NO_JITTER=1` to disable jitter entirely.

## One-shot vs recurring 鈥?when to pick which
...
Cron tasks live in the current eddy CLI session. When you exit, they
are persisted under the session homedir; the next `eddy resume` of the
...
Tasks do **not** carry over into a brand-new session 鈥?they are scoped
