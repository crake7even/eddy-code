 ...
 *
 * `wallNow` resolution is driven by the `EDDY_CRON_CLOCK` env var; see
...
export interface ClockSources {
   ...
   * via `EDDY_CRON_CLOCK`. Used for cron matching, `createdAt`, stale
   * judgment.
   */
  wallNow(): number;
 ...
 * whenever `EDDY_CRON_CLOCK` is unset, set to `"system"`, or set to a
...
function debugInvalidSpec(spec: string, reason: string): void {
  ...
  // gated on EDDY_CRON_DEBUG is enough — production is silent.
  if (process.env['EDDY_CRON_DEBUG'] === '1') {
    process.stderr.write(
      `[cron/clock] invalid EDDY_CRON_CLOCK spec ${JSON.stringify(spec)}: ${reason} — falling back to system clock\n`,
    );
  }
}
