 ...
 *     mid-turn we skip firing 鈥?but we do NOT advance `lastSeenAt`. The
 ...
 *     ideal fires we collapsed into it. Floor at 1 鈥?every fire that
 ...
 *     defend against re-entrant ticks within the same call stack 鈥?a
 ...
 *     stderr trace gated on `EDDY_CRON_DEBUG=1`) so one busted cron
...
export interface CronSchedulerOptions {
   ...
   * Called every tick 鈥?keep cheap.
   */
   ...
   * firing but does not lose tasks 鈥?the next idle tick fires them
   ...
   * `eddy resume` does not replay the fire.
   *
   ...
   *   - undefined (default) 鈫?1000ms.
   *   - 0 or null 鈫?no automatic polling. Caller drives tick()
   *     manually.
   *
   * Used by P1.8 to wire `EDDY_CRON_MANUAL_TICK=1` to disable the
   * timer.
   */
...
export interface CronScheduler {
  /** Begin the auto-tick loop. Idempotent 鈥?calling twice is a no-op. */
  start(): void;
 ...
 * minutes (~7 days). Beyond that we'd rather report 10 000 than spin 鈥?
...
export function createCronScheduler(opts: CronSchedulerOptions): CronScheduler {
  ...
  // Now persisted across `eddy resume` via `task.lastFiredAt`: when
   ...
   * **jittered delivery time** is also 鈮?`nowMs`. Returns the count
   * plus the timestamp of the last ideal occurrence that satisfied the
   * jittered-due test 鈥?used by the caller as the new `lastSeenAt`
   ...
   *
   * Always returns at least 1 鈥?every actual fire is one occurrence.
  ...
  function countCoalesced(
    ...
    while (count < MAX_COALESCE_ITERATIONS) {
      ...
      // past `nowMs` would leak the occurrence 鈥?the caller advances
    ...
    try {
      for (const task of tasks) {
        try {
          ...
          // skew, mis-set bench env) and ignored 鈥?never trust a
          ...
          // Due 鈥?compute coalescedCount starting from the first
          // ideal fire (not the jittered one 鈥?jitter only shifts the
          ...
          if (!delivered) {
            // Leave lastSeenAt/store untouched 鈥?next tick will
          ...
          if (task.recurring === false) {
            ...
            // *delivered* timestamp 鈥?rather than `now` 鈥?keeps any
            ...
            // through to disk. Fire-and-forget 鈥?the callback is
  ...
  function start(): void {
    ...
    // Don't keep the event loop alive on the scheduler alone 鈥?the
