...
const EDDY_PLUGIN_ROOT_PATH = 'eddy.plugin.json';
const EDDY_PLUGIN_DIR_PATH = '.eddy-plugin/plugin.json';
...
// Eddy CLI). We do not run them; emit an info diagnostic so plugin authors and
...
export async function parseManifest(pluginRoot: string): Promise<ParsedManifestResult> {
  const rootJsonPath = path.join(pluginRoot, EDDY_PLUGIN_ROOT_PATH);
  const dirJsonPath = path.join(pluginRoot, EDDY_PLUGIN_DIR_PATH);
  ...
  if (!rootJsonExists && !dirJsonExists && !claudeDirJsonExists) {
    // Fallback 1: bare SKILL.md at repo root (common agent skill pattern).
    ...
    // forcing authors to add EddyCode-specific packaging.
    ...
    return {
      diagnostics: [
        {
          severity: 'error',
          message: `No manifest at ${EDDY_PLUGIN_ROOT_PATH}, ${EDDY_PLUGIN_DIR_PATH}, or ${CLAUDE_PLUGIN_DIR_PATH}`,
        },
      ],
    };
  }

  // Priority: eddy.plugin.json > .eddy-plugin/plugin.json > .claude-plugin/plugin.json
...
function recordUnsupportedRuntimeFields(
  ...
  for (const field of UNSUPPORTED_RUNTIME_FIELDS) {
    ...
    diagnostics.push({
      severity: 'info',
      message: `"${field}" is present but not supported by Eddy plugins`,
    });
  }
}
