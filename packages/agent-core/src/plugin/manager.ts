...
// Hidden Eddy CLI subcommand that re-enters as a Node interpreter.
// Used as fallback when an MCP server declares `"command": "node"` but the
// user is running a single-binary Eddy build that doesn't have `node` on PATH.
const EDDY_NODE_FALLBACK_SUBCOMMAND = '__plugin_run_node';
...
export class PluginManager {
  private readonly eddyHomeDir: string;
  ...
  async load(): Promise<void> {
    const file = await readInstalled(this.eddyHomeDir);
  ...
  async install(source: string): Promise<PluginRecord> {
    ...
    if (resolved.kind === 'local-path') {
      ...
      normalizedRoot = await copyPluginToManagedRoot(this.eddyHomeDir, id, sourceRoot);
      ...
      const tmpDir = await mkdtemp(path.join(tmpdir(), 'eddy-plugin-zip-'));
      try {
        ...
        id = normalizePluginId(parsed.manifest.name);
        normalizedRoot = await copyPluginToManagedRoot(this.eddyHomeDir, id, detectedRoot);
        parsed = await parseManifest(normalizedRoot);
      } finally {
  ...
  private async persist(): Promise<void> {
    ...
    await writeInstalled(this.eddyHomeDir, { version: 1, plugins: installed });
  }
...
async function copyPluginToManagedRoot(
  ...
  const managedRoot = path.join(eddyHomeDir, 'plugins', 'managed', id);
...
function withPluginMcpRuntime(
  ...
  const env = {
    ...config.env,
    EDDY_CODE_HOME: eddyHomeDir,
    EDDY_PLUGIN_ROOT: pluginRoot,
  };

  if (config.command === 'node' && isEddyNativeBinary()) {
    return {
      ...config,
      command: process.execPath,
      args: [EDDY_NODE_FALLBACK_SUBCOMMAND, ...(config.args ?? [])],
...
function isEddyNativeBinary(): boolean {
