...
用户调用了 `/make-skill`。你的任务是通过对话引导用户，把当前会话中解决问题的方式沉淀为一个可复用的 Skill，并安装到插件中心。
...
- 文件清单（至少包含 `SKILL.md`）
- 适用场景
- 安装位置：`~/.eddy-code/plugins/managed/<name>/`，可通过 `/plugin` 管理
...
- 成功：`Skill 已安装到 ~/.eddy-code/plugins/managed/<name>/。新会话中可通过 /<name> 调用。`
- 失败：说明错误原因，不要重试。
