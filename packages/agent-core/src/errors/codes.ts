/**
 * Error codes for Eddy Core's public error protocol.
 *
 * `ErrorCodes` is the source of truth for every code Eddy Core may emit.
...
export type EddyErrorCode = (typeof ErrorCodes)[keyof typeof ErrorCodes];

export interface EddyErrorInfo {
...
export const EDDY_ERROR_INFO = {
  ...
  'plugin.load_failed': {
    ...
    retryable: true,
    public: true,
    action: 'Fix the installed.json file under $EDDY_CODE_HOME/plugins/ and run /plugins reload.',
  },
  ...
  'shell.git_bash_not_found': {
    ...
    retryable: false,
    public: true,
    action: 'Install Git for Windows from https://gitforwindows.org/ or set EDDY_SHELL_PATH to a bash.exe.',
  },
...
} as const satisfies Record<EddyErrorCode, EddyErrorInfo>;
