 ...
 *   { id: 'my-feature', env: 'EDDY_CODE_EXPERIMENTAL_MY_FEATURE', default: false, surface: 'both' }
 *
 * Keep the `as const satisfies` — it derives the literal `FlagId` union that gives `enabled()`
 * autocomplete and typo-checking. `env` must start with 'EDDY_CODE_EXPERIMENTAL_', be unique, and
 * not equal the master switch 'EDDY_CODE_EXPERIMENTAL_FLAG'; `id` must not be 'flag'.
 */
export const FLAG_DEFINITIONS = [
  {
    id: 'micro-compaction',
    env: 'EDDY_CODE_EXPERIMENTAL_MICRO_COMPACTION',
    default: false,
    surface: 'both',
  },
  {
    id: 'wolfpack',
    env: 'EDDY_CODE_EXPERIMENTAL_WOLFPACK',
    default: false,
    surface: 'both',
  },
