...
import { inputTotal } from '@eddy-code/ltod';
...
describe('runTurn — turn lifecycle', () => {
  ...
  it('throws EddyError(loop.max_steps_exceeded) when steps reach maxSteps', async () => {
    const echo = new EchoTool();
    ...
    expect((error as EddyError).code).toBe(ErrorCodes.LOOP_MAX_STEPS_EXCEEDED);
    expect((error as EddyError).details).toEqual({ maxSteps: 2 });
