 ...
 * the in-memory store on `eddy resume`. The scheduler's
 * `createdAt`-based baseline is what makes a reloaded task fire
 * correctly even when ideal fire times landed during downtime 鈥?these
...
beforeEach(async () => {
  ...
  sessionDir = await mkdtemp(join(tmpdir(), 'eddy-cron-resume-'));
});
...
describe('CronManager 鈥?persistence and resume', () => {
  ...
  it('recurring task missed during downtime fires once with coalescedCount > 1', async () => {
    ...
    // Session B: 23 minutes later (鈮?4 ideal fires missed: t+5, +10,
  ...
  it('one-shot scheduled in the past fires once on resume and the file is removed', async () => {
    ...
    // Session B: 10 minutes after the anchor 鈥?the one-shot's ideal
    // fire (anchor + 100s 鈮?22:15:00) is in the past.
    const stubB = createAgentStub({ homedir: sessionDir });
  ...
  it('recurring task fired before shutdown does NOT replay on resume', async () => {
    ...
    // Advance ~6 minutes 鈥?the first */5 ideal (anchor + 100s) is now
