    ...
    skills: {
      registry: {
        ...
        renderSkillPrompt: (skill) => {
          ...
          return `<eddy-plugin-instructions plugin="${plugin.id}">\n${instructions}\n</eddy-plugin-instructions>\n\n${skill.content}`;
        },
      },
    },
    log: {
...
describe('PluginSessionStartInjector', () => {
  it('injects one <plugin_session_start> block per declared sessionStart on first call', async () => {
    ...
    expect(text).toContain('<eddy-plugin-instructions plugin="superpowers">');
  ...
  it('does not hard-code Superpowers guidance when the skill has no plugin instructions', async () => {
    ...
    expect(text).not.toContain('<eddy-plugin-instructions plugin="superpowers">');
