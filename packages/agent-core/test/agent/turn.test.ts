...
describe('Agent turn flow', () => {
  ...
  it('emits a friendly model.not_configured error when no model is configured', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] turn.ended               { "turnId": 0, "reason": "failed", "error": { "code": "model.not_configured", "message": "No model configured. Run \`eddy config\` or use \`/model\` to set a default model.", "name": "EddyError", "details": { "turnId": 0 }, "retryable": false } }
    `);
    expect(ctx.newEvents()).toMatchInlineSnapshot(
      `[emit] error   { "code": "model.not_configured", "message": "No model configured. Run \`eddy config\` or use \`/model\` to set a default model.", "name": "EddyError", "details": { "turnId": 0 }, "retryable": false }`,
    );
  });
  ...
  it('continues a turn when the model tries to stop with unverified working-set paths', async () => {
    const ctx = testAgent();
    ctx.configure();
  ...
  it('cancels while waiting for a Stop hook', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'eddy-stop-hook-'));
  ...
  it('cancels while waiting for a PreToolUse hook inside permission evaluation', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'eddy-pre-tool-hook-'));
    const marker = join(dir, 'started');
    const script = [
      "const fs=require('node:fs');",
      `fs.writeFileSync(${JSON.stringify(marker)}, 'started');`,
