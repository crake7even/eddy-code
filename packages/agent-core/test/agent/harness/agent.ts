...
import { type Environment, type Jian, type JianProcess } from '@eddy-code/jian';
import type { ModelCapability, ProviderConfig } from '@eddy-code/ltod';
...
export interface TestAgentOptions {
  ...
  readonly initialConfig?: EddyConfig;
...
export class AgentTestContext {
  ...
  constructor(options: TestAgentOptions = {}) {
    ...
    // by this harness leaks one listener 鈥?Node prints a
  ...
  configureRuntimeModel(
    ...
    if (this.options.providerManager === undefined) {
      this.eddyConfig = configWithProvider(this.eddyConfig, provider, modelCapabilities);
    }
...
function emptyConfig(): EddyConfig {
...
function providerConfigForAlias(provider: ProviderConfig): EddyConfig['providers'][string] {
  return {
    type: provider.type,
