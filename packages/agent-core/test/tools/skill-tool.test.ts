...
describe('SkillTool execution', () => {
  ...
  it('records inline skill content as a system reminder', async () => {
    ...
    expect(methods.recordSystemReminder.mock.calls[0]?.[0]).toContain(
      '<eddy-skill-loaded name="commit" args="message text">\nbody of commit\n\nARGUMENTS: message text\n</eddy-skill-loaded>',
    );
  });
  ...
  it('keeps plugin instructions adjacent to model-invoked skill content', async () => {
    ...
    expect(methods.recordSystemReminder.mock.calls[0]?.[0]).toContain(
      '<eddy-skill-loaded name="brainstorming" args="">\n' +
        '<eddy-plugin-instructions plugin="superpowers">\n' +
  ...
  it('expands skill body placeholders for model-invoked inline skills', async () => {
    ...
    expect(methods.recordSystemReminder.mock.calls[0]?.[0]).toContain(
      '<eddy-skill-loaded name="commit" args="-m &quot;fix login&quot;">\nFlag: -m\nCommit message: fix login\nRaw: -m "fix login"\n</eddy-skill-loaded>',
    );
  ...
  it('expands session id from the skill registry for model-invoked skills', async () => {
    const methods = skillToolMethods();
    const tool = skillTool(
      registry([skill('session-aware', {}, 'Session: ${EDDY_SESSION_ID}')], {
    ...
    expect(methods.recordSystemReminder.mock.calls[0]?.[0]).toContain(
      '<eddy-skill-loaded name="session-aware" args="">\nSession: ses_model_skill\n</eddy-skill-loaded>',
    );
  });
  ...
  it('escapes skill name and args in the wrapper boundaries', async () => {
    ...
    expect(methods.recordSystemReminder.mock.calls[0]?.[0]).toContain(
      '<eddy-skill-loaded name="a&amp;b" args="&lt;raw &quot;value&quot;&gt;">\nbody of a&b\n\nARGUMENTS: <raw "value">\n</eddy-skill-loaded>',
    );
    expect(methods.recordSkillActivation).toHaveBeenCalledTimes(1);
  });
