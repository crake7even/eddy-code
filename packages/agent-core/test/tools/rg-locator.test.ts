...
describe('findExistingRg', () => {
  ...
  beforeEach(() => {
    fakeShare = join(tmpdir(), `eddy-rg-${String(Date.now())}-${String(Math.random()).slice(2)}`);
    mkdirSync(join(fakeShare, 'bin'), { recursive: true });
    savedPath = process.env['PATH'];
    // Empty PATH 鈫?rules out step 1 (system-path) for the default case.
    process.env['PATH'] = '';
  });
  afterEach(() => {
...
describe('verifyArchiveChecksum', () => {
  let fakeDir: string;
  beforeEach(() => {
    fakeDir = join(tmpdir(), `eddy-rg-sha-${String(Date.now())}-${String(Math.random()).slice(2)}`);
...
describe('ensureRgPath download branch', () => {
  ...
  beforeEach(() => {
    fakeShare = join(
      tmpdir(),
      `eddy-rg-dl-${String(Date.now())}-${String(Math.random()).slice(2)}`,
    );
    mkdirSync(join(fakeShare, 'bin'), { recursive: true });
    savedPath = process.env['PATH'];
...
// 鈹€鈹€ Windows zip download branch 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
//
...
describe('ensureRgPath Windows download branch', () => {
  ...
  beforeEach(() => {
    fakeShare = join(
      tmpdir(),
      `eddy-rg-win-${String(Date.now())}-${String(Math.random()).slice(2)}`,
    );
    mkdirSync(join(fakeShare, 'bin'), { recursive: true });
    savedPath = process.env['PATH'];
    ...
    savedArch = process.arch;
    savedPlatform = process.platform;
    // Simulate a Windows host end-to-end 鈥?`rgBinaryName()`, `whichRg()`
  ...
  it('throws with "CDN content may have changed" when the zip omits rg.exe', async () => {
    // Archive is well-formed but holds the wrong entry 鈥?mirrors the
