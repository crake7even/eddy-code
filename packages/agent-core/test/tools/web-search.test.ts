...
import { EddyCliWebSearchProvider } from '../../src/tools/providers/eddy-cli-web-search';
  ...
  it('classifies authentication failures', async () => {
    const provider: WebSearchProvider = {
      search: vi
        .fn()
        .mockRejectedValue(
          new Error('EddyCli search request failed: HTTP 401 (auth/unauthorized).'),
        ),
    };
    const tool = new WebSearchTool(provider);
    const result = await executeTool(tool, {
...
describe('EddyCliWebSearchProvider', () => {
  it('does not force-refresh request auth after a 401 response', async () => {
    ...
    const provider = new EddyCliWebSearchProvider({
