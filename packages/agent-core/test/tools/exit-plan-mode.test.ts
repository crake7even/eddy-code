    ...
    planMode: {
      ...
      data: vi.fn(async () => {
        ...
        return {
          ...
          path: input.path ?? '/tmp/eddy-plan.md',
        };
      }),
      exit: () => {
...
describe('ExitPlanModeTool', () => {
  ...
  it('exits with the current plan without consulting permission approval', async () => {
    ...
    expect(result.output).toContain('Plan saved to: /tmp/eddy-plan.md');
  ...
  it('returns an error when no plan content is available', async () => {
    ...
    expect(result).toMatchObject({ isError: true });
    expect(result.output).toContain('Write your plan to /tmp/eddy-plan.md first');
    expect(emit).not.toHaveBeenCalled();
  });
