...
describe('sanitizeSkillName', () => {
  ...
  it('rejects empty names', () => {
    expect(() => sanitizeSkillName('')).toThrow(EddyError);
  });
...
describe('writeSkillPackage', () => {
  it('writes SKILL.md and supporting files under the user scope', async () => {
    ...
    expect(normalizePathText(result.targetDir)).toBe(
      normalizePathText(join(tmp, '.eddy-code', 'skills', 'test-skill')),
    );
...
describe('MakeSkillApplyTool', () => {
  it('writes the skill package to the plugin center and reports the target directory', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'make-skill-test-'));
    const jian = await makeJian(tmp);
    const agent = { jian, eddyHomeDir: tmp } as unknown as Agent;
    ...
    expect(result.isError).not.toBe(true);
    expect(normalizePathText(typeof result.output === 'string' ? result.output : '')).toContain(
      normalizePathText(join(tmp, 'plugins', 'managed', 'applied-skill')),
    );

    const manager = new PluginManager({ eddyHomeDir: tmp });
    await manager.load();
