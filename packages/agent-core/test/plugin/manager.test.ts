...
async function makeEddyHome(): Promise<string> {
  return mkdtemp(path.join(tmpdir(), 'eddy-home-'));
}
...
async function makePlugin(
  ...
  await writeFile(
    path.join(root, 'eddy.plugin.json'),
...
describe('PluginManager', () => {
  it('install() adds a plugin and load() rehydrates it from disk', async () => {
    const home = await makeEddyHome();
    const pluginRoot = await makePlugin('demo', { skills: true });

    let manager = new PluginManager({ eddyHomeDir: home });
    await manager.load();
    ...
    manager = new PluginManager({ eddyHomeDir: home });
    await manager.load();
    expect(manager.list()).toHaveLength(1);
  ...
  it('install() accepts a .eddy-plugin manifest', async () => {
    const home = await makeEddyHome();
    const root = await mkdtemp(path.join(tmpdir(), 'eddy-plugin-'));
    await mkdir(path.join(root, '.eddy-plugin'), { recursive: true });
    await mkdir(path.join(root, 'skills'), { recursive: true });
    await writeFile(
      path.join(root, '.eddy-plugin', 'plugin.json'),
      JSON.stringify({
        name: 'superpowers',
        skills: './skills/',
    ...
    const manager = new PluginManager({ eddyHomeDir: home });
    await manager.load();
    ...
    expect(record.manifestKind).toBe('eddy-plugin-dir');
    ...
    expect(manager.pluginSkillRoots()).toContainEqual({
      ...
      source: 'extra',
      plugin: { id: 'superpowers', instructions: 'Use Eddy tools.' },
    });
  });
  ...
  it('install() copies a symlinked plugin root into the managed plugins dir', async () => {
    ...
    const reloaded = new PluginManager({ eddyHomeDir: home });
    await reloaded.load();
  ...
  it('reload() picks up edits to the managed plugin copy', async () => {
    ...
    await writeFile(
      path.join(managedRoot, 'eddy.plugin.json'),
  ...
  it('keeps a plugin in error state instead of losing it on a broken manifest', async () => {
    ...
    await manager.install(root);
    await writeFile(
      path.join(await managedPluginRoot(home, 'demo'), 'eddy.plugin.json'),
      '{ not json',
      'utf8',
    );
    ...
    expect(manager.enabledMcpServers()).toEqual(
      expect.objectContaining({
        'plugin-demo:finance': expect.objectContaining({
          command: 'finance-mcp',
          cwd: managedRoot,
          env: expect.objectContaining({ EDDY_CODE_HOME: home, EDDY_PLUGIN_ROOT: managedRoot }),
        }),
  ...
  it('install() from zip-url overwrites existing zip-url plugin', async () => {
    ...
    const zipBuffer1 = await createZipBuffer([
      { name: 'plugin/eddy.plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '1.0.0' }) },
    ]);
    ...
    const zipBuffer2 = await createZipBuffer([
      { name: 'plugin/eddy.plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '2.0.0' }) },
    ]);
  ...
  it('install() from github URL resolves latest release and records github metadata', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-abc/eddy.plugin.json',
  ...
  it('install() from /tree/<tag-shaped-ref> downloads via short form, not refs/heads/ (P1 regression)', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'obra-superpowers-v5.1.0/eddy.plugin.json',
  ...
  it('install() from /releases/tag/<tag> resolves precisely via refs/tags/', async () => {
    ...
    try {
      ...
      // Explicit tag origin 鈫?kind is 'tag', URL uses refs/tags/ for
  ...
  it('install() from github /tree/<branch> bypasses the GitHub API', async () => {
    ...
    const zipBuffer = await createZipBuffer([
      {
        name: 'wbxl2000-superpowers-main/eddy.plugin.json',
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    const cdnZip = await createZipBuffer([
      { name: 'pkg/eddy.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.0.0' }) },
    ]);
    ...
    const ghZip = await createZipBuffer([
      { name: 'pkg/eddy.plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.1.0' }) },
    ]);
    using _ = mockGithubFetch({
      releaseTag: 'v5.1.0',
      tarball: ghZip,
