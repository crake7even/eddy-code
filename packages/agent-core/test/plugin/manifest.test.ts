...
async function makePlugin(
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'eddy-plugin-test-'));
...
describe('parseManifest', () => {
  it('reads a minimal eddy.plugin.json at the plugin root', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'demo', version: '1.0.0' }),
    });
    const result = await parseManifest(root);
    expect(result.manifest?.name).toBe('demo');
    expect(result.manifest?.version).toBe('1.0.0');
    expect(result.manifestKind).toBe('eddy-plugin-root');
    expect(result.diagnostics).toEqual([]);
  });

  it('prefers root eddy.plugin.json when .eddy-plugin/plugin.json also exists', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'root-version', version: '1.0.0' }),
      '.eddy-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
    expect(result.manifestKind).toBe('eddy-plugin-root');
    expect(result.manifest?.name).toBe('root-version');
    expect(result.shadowedManifestPath).toBe(path.join(root, '.eddy-plugin/plugin.json'));
  });

  it('falls back to .eddy-plugin/plugin.json when eddy.plugin.json is absent', async () => {
    const root = await makePlugin(
      {
        '.eddy-plugin/plugin.json': JSON.stringify({
          name: 'demo',
          version: '1.0.0',
          keywords: ['workflow'],
          skills: './skills/',
          ...
          skillInstructions: 'Use Eddy tools.',
        }),
      },
      { dirs: ['skills'] },
    );
    const result = await parseManifest(root);
    expect(result.manifestKind).toBe('eddy-plugin-dir');
    expect(result.manifestPath).toBe(path.join(root, '.eddy-plugin/plugin.json'));
    expect(result.manifest?.name).toBe('demo');
    expect(result.manifest?.version).toBe('1.0.0');
    ...
    expect(result.manifest?.skillInstructions).toBe('Use Eddy tools.');
  });

  it('does NOT fall back to .eddy-plugin/plugin.json when eddy.plugin.json is invalid JSON', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': '{ not json',
      '.eddy-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
  ...
  it('rejects names that violate the regex', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'Bad Name!' }),
    });
    const result = await parseManifest(root);
    expect(result.manifest).toBeUndefined();
    expect(result.diagnostics).toContainEqual(
  ...
  it('resolves a single skills path', async () => {
    const root = await makePlugin(
      { 'eddy.plugin.json': JSON.stringify({ name: 'demo', skills: './skills/' }) },
      { dirs: ['skills'] },
    );
    const result = await parseManifest(root);
    expect(result.manifest?.skills).toEqual([path.join(root, 'skills')]);
  ...
  it('resolves an array of skills paths', async () => {
    const root = await makePlugin(
      {
        'eddy.plugin.json': JSON.stringify({
          name: 'demo',
          skills: ['./a/', './b/'],
        }),
      },
  ...
  it('rejects a skills path that escapes plugin_root', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'demo', skills: './../escape' }),
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
      expect.objectContaining({
  ...
  it('rejects a skills path that escapes via a symlink', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'demo', skills: './sym' }),
    });
    const outside = await mkdtemp(path.join(tmpdir(), 'eddy-plugin-outside-'));
  ...
  it('warns when skills resolves to a non-directory', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'demo', skills: './notes.md' }),
      'notes.md': 'hi',
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
  ...
  it('falls back to root SKILL.md when skills field is absent', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'demo' }),
      'SKILL.md': '---\nname: root-skill\n---\nbody',
    });
    const result = await parseManifest(root);
  ...
  it('parses skillInstructions', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'demo', skillInstructions: 'Do this.' }),
    });
    const result = await parseManifest(root);
  ...
  it('parses keywords metadata', async () => {
    const root = await makePlugin({
      'eddy.plugin.json': JSON.stringify({ name: 'demo', keywords: ['finance', 'workflow'] }),
    });
    const result = await parseManifest(root);
