...
describe('extractZip', () => {
  ...
  it('prefers a plugin manifest at the archive root', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'eddy.plugin.json', data: '{"name":"root"}' },
      { name: 'examples/demo/eddy.plugin.json', data: '{"name":"demo"}' },
    ]);
  ...
  it('detects plugin root with eddy.plugin.json', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'my-plugin/eddy.plugin.json', data: '{"name":"test"}' },
      { name: 'my-plugin/readme.md', data: '# Test' },
    ]);

    const root = await extractZip(zipBuffer, destDir);
    expect(root).toBe(path.join(destDir, 'my-plugin'));
    const manifest = await readFile(path.join(root, 'eddy.plugin.json'), 'utf8');
    expect(manifest).toBe('{"name":"test"}');
  });

  it('detects plugin root with .eddy-plugin/plugin.json', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'my-plugin/.eddy-plugin/plugin.json', data: '{"name":"test"}' },
      { name: 'my-plugin/skills/demo/SKILL.md', data: '---\nname: demo\n---\nbody' },
    ]);

    const root = await extractZip(zipBuffer, destDir);
    expect(root).toBe(path.join(destDir, 'my-plugin'));
    const manifest = await readFile(path.join(root, '.eddy-plugin', 'plugin.json'), 'utf8');
    expect(manifest).toBe('{"name":"test"}');
  });
  ...
  it('detects a single wrapper directory before nested manifests', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'outer/eddy.plugin.json', data: '{"name":"outer"}' },
      { name: 'outer/inner/eddy.plugin.json', data: '{"name":"inner"}' },
    ]);
