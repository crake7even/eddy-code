...
import type { ProviderConfig } from '@eddy-code/ltod';
...
describe('McpConnectionManager', () => {
  ...
  it('emits status transitions in order (pending 鈫?connected) per server', async () => {
    const cm = new McpConnectionManager();
  ...
  it('reconnect throws a coded EddyError when the server name is unknown', async () => {
    const cm = new McpConnectionManager();
    try {
      await expect(cm.reconnect('nope')).rejects.toBeInstanceOf(EddyError);
  ...
  it('reconnect rejects disabled servers without connecting them', async () => {
    ...
    try {
      ...
      await expect(reconnect).rejects.toBeInstanceOf(EddyError);
  ...
  it('flips HTTP servers into needs-auth when the server returns 401 and no static token is set', async () => {
    ...
    const storeDir = await mkdtemp(join(tmpdir(), 'eddy-mcp-oauth-cm-'));
  ...
  it('flips cached OAuth credentials that require reauth into needs-auth', async () => {
    ...
    const storeDir = await mkdtemp(join(tmpdir(), 'eddy-mcp-oauth-cached-'));
    const oauthService = new McpOAuthService({ store: new JsonFileStore(storeDir) });
    ...
    try {
      await cm.connectAll({
        crashy: {
          ...
          env: { EDDY_TEST_MCP_EXIT_AFTER_MS: '500', EDDY_TEST_MCP_STDERR: 'fatal: out of memory' },
    ...
    try {
      await cm.connectAll({
        nope: {
          ...
          env: { EDDY_TEST_MCP_STDERR: 'fatal: missing API token EDDY_X' },
          startupTimeoutMs: 4_000,
        },
      });
      const entry = cm.get('nope');
      expect(entry?.status).toBe('failed');
      expect(entry?.error).toContain('fatal: missing API token EDDY_X');
    } finally {
      await cm.shutdown();
    }
  }, 10000);
  ...
  it('flips connected HTTP servers to failed when the SDK reports a terminal transport error', async () => {
    ...
    // report a dead transport 鈥?there is no `onclose` for that case).
...
describe('Session MCP startup', () => {
  it('stores default MCP OAuth credentials under the configured Eddy home', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'eddy-session-mcp-oauth-home-'));
    const processHome = join(tmp, 'process-home');
    const eddyHome = join(tmp, 'eddy-home');
    ...
    try {
      ...
      await expect(
        readFile(
          join(eddyHome, 'credentials', 'mcp', `${provider.storeKey}-tokens.json`),
          'utf-8',
        ),
      ).resolves.toContain('session-token');
      await expect(
        readFile(
          join(processHome, '.eddy-code', 'credentials', 'mcp', `${provider.storeKey}-tokens.json`),
          'utf-8',
        ),
  ...
  it('does not block main agent creation on slow MCP startup', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'eddy-session-mcp-startup-'));
    const session = new Session({
      id: 'test-mcp-slow',
      jian: testJian.withCwd(tmp),
      homedir: join(tmp, 'session'),
  ...
  it('waits for initial MCP startup before the first prompt reaches the model', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'eddy-session-mcp-prompt-'));
      ...
      mcpConfig: {
        servers: {
          slow: {
            ...
            command: process.execPath,
            args: [stdioFixture],
            env: { EDDY_TEST_MCP_START_DELAY_MS: '250' },
            startupTimeoutMs: 2_000,
          },
        },
      },
  ...
  it('emits tool.list.updated(mcp.disconnected) when reconnect drops the live tools', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'eddy-session-mcp-reconnect-'));
    const events: SessionRpcEvent[] = [];
    const session = new Session({
      id: 'test-mcp-mixed',
      jian: testJian.withCwd(tmp),
    ...
    try {
      ...
      // The reconnect cycle: pending (tools cleared) 鈫?connected (tools back).
