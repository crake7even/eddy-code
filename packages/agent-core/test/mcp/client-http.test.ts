...
function expectConfigInvalid(fn: () => unknown): void {
  try {
    ...
    expect((error as EddyError).code).toBe(ErrorCodes.CONFIG_INVALID);
    return;
  }
...
describe('buildMcpHttpHeaders', () => {
  ...
  it('passes through configured static headers', () => {
    expect(
      buildMcpHttpHeaders(
        { transport: 'http', url: 'https://x', headers: { 'X-Tenant': 'eddy' } },
        () => undefined,
      ),
    ).toEqual({ 'X-Tenant': 'eddy' });
  });
  ...
  it('throws EddyError(config.invalid) when a configured bearer token env var is empty or missing', () => {
    expectConfigInvalid(() =>
      buildMcpHttpHeaders(
...
describe('HttpMcpClient', () => {
  ...
  it('flips to unexpected-close when the SDK signals a terminal transport error', async () => {
    ...
    try {
      ...
      // (e.g. "Maximum reconnection attempts (3) exceeded.") 鈥?there is no
  ...
  it('ignores transient SDK errors that the transport recovers from', async () => {
    ...
    try {
      ...
      // SSE flap that the SDK will retry on its own 鈥?should NOT flip the
