...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'eddy-mcp-loader-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('resolveMcpJsonPaths', () => {
  it('returns the canonical user and project paths', () => {
    const paths = resolveMcpJsonPaths({ cwd: '/work/proj', homeDir: '/home/user/.eddy-code' });
    expect(paths.user).toBe('/home/user/.eddy-code/mcp.json');
    expect(paths.project).toBe('/work/proj/.eddy-code/mcp.json');
  });
});
...
describe('loadMcpServers', () => {
  ...
  it('merges project-local mcp.json with user-global, project overriding on conflict', async () => {
    ...
    await writeJson(join(cwd, '.eddy-code', 'mcp.json'), {
      mcpServers: {
  ...
  it('throws EddyError(config.invalid) on invalid JSON', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeFile(join(home, 'mcp.json'), '{not json}', 'utf-8');
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toBeInstanceOf(EddyError);
    await expect(loadMcpServers({ cwd, homeDir: home })).rejects.toMatchObject({
      code: ErrorCodes.CONFIG_INVALID,
    });
  });

  it('throws EddyError(config.invalid) on schema violation (unknown transport)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('throws EddyError(config.invalid) on schema violation (missing required field)', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
  ...
  it('honors EDDY_CODE_HOME env var when homeDir is not supplied', async () => {
    const home = makeTempDir();
    const cwd = makeTempDir();
    await writeJson(join(home, 'mcp.json'), {
    ...
    process.env['EDDY_CODE_HOME'] = home;
    try {
