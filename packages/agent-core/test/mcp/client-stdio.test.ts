...
describe('StdioMcpClient', () => {
  it('rejects unsupported executor at construction time', () => {
    expect(
      ...
      expect.objectContaining({ name: 'EddyError', code: 'not_implemented' }) as unknown as Error,
    );
    ...
    expect(thrown).toBeInstanceOf(EddyError);
  });
  ...
  it('forwards configured env to the spawned server', async () => {
    const client = new StdioMcpClient({
      ...
      command: process.execPath,
      args: [fixture],
      env: { EDDY_TEST_ENV: 'forwarded-value' },
    });
    try {
      await client.connect();
      const result = await client.callTool('read_env', { name: 'EDDY_TEST_ENV' });
  ...
  it('forwards allowlisted env vars; config.env overrides on conflict', async () => {
    ...
    const parentOnly = `TERM_EDDY_TEST_PARENT_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const shared = `TERM_EDDY_TEST_SHARED_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  it('does not forward non-allowlisted env vars', async () => {
    const secret = `EDDY_TEST_SECRET_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  it('captures recent stderr into a snapshot the manager can attach to errors', async () => {
    const banner = `eddy-test-stderr-${Date.now()}`;
    const client = new StdioMcpClient({
      transport: 'stdio',
      command: process.execPath,
      args: [stderrThenExitFixture],
      env: { EDDY_TEST_MCP_STDERR: banner },
    });
    try {
  ...
  it('notifies an unexpected-close listener when the child exits after connect', async () => {
    const banner = `eddy-test-crash-${Date.now()}`;
    const client = new StdioMcpClient({
      transport: 'stdio',
      command: process.execPath,
      args: [crashAfterConnectFixture],
      env: { EDDY_TEST_MCP_EXIT_AFTER_MS: '50', EDDY_TEST_MCP_STDERR: banner },
    });
  ...
  it('buffers an early close and replays it on listener registration', async () => {
    const banner = `eddy-test-early-${Date.now()}`;
    const client = new StdioMcpClient({
      transport: 'stdio',
      command: process.execPath,
      args: [crashAfterConnectFixture],
      env: { EDDY_TEST_MCP_STDERR: banner, EDDY_TEST_MCP_EXIT_CODE: '0' },
    });
    try {
      await client.connect();
