...
import { EddyError } from '../../src/errors';
...
function expectConfigInvalid(fn: () => unknown): void {
  try {
    ...
    expect((error as EddyError).code).toBe('config.invalid');
    return;
  }
...
const MIN = {
  EDDY_MODEL_NAME: 'eddy-for-coding',
  EDDY_MODEL_API_KEY: 'sk-test',
  EDDY_MODEL_MAX_CONTEXT_SIZE: '262144',
} as const;

describe('applyEnvModelConfig', () => {
  it('returns the config unchanged when EDDY_MODEL_NAME is absent', () => {
    const base = getDefaultConfig();
    expect(applyEnvModelConfig(base, {})).toBe(base);
  });

  it('throws when EDDY_MODEL_NAME is set but API key is missing', () => {
    expectConfigInvalid(() => apply({ EDDY_MODEL_NAME: 'm' }));
  });

  it('defaults max_context_size to 262144 (256K) when unset', () => {
    expect(
      apply({ EDDY_MODEL_NAME: 'm', EDDY_MODEL_API_KEY: 'k' })
  ...
  it.each(['abc', '0', '1.5', '-1'])(
    ...
    (value) => {
      expectConfigInvalid(() =>
        apply({ ...MIN, EDDY_MODEL_MAX_CONTEXT_SIZE: value }),
      );
    },
  );

  it('synthesizes a eddy provider and model from the minimal set', () => {
  ...
  it('applies provider type and its default base url', () => {
    expect(apply({ ...MIN, EDDY_MODEL_PROVIDER_TYPE: 'openai' })
      .providers[ENV_MODEL_PROVIDER_KEY]).toMatchObject({
      type: 'openai',
      baseUrl: 'https://api.openai.com/v1',
    });
    const anthropic = apply({ ...MIN, EDDY_MODEL_PROVIDER_TYPE: 'anthropic' })
  ...
  it('rejects unsupported provider types', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, EDDY_MODEL_PROVIDER_TYPE: 'google-genai' }),
    );
  });

  it('lets an explicit base url override the default', () => {
    expect(
      apply({ ...MIN, EDDY_MODEL_BASE_URL: 'https://api-staff.msh.team/v1' })
  ...
  it('parses comma-separated capabilities (trimmed, lowercased)', () => {
    expect(
      apply({ ...MIN, EDDY_MODEL_CAPABILITIES: 'Image_In, thinking ,' })
  ...
  it('sets display_name only when provided', () => {
    ...
    expect(
      apply({ ...MIN, EDDY_MODEL_DISPLAY_NAME: 'Staff Model' })
  ...
  it('writes type-specific fields and validates max_output_size', () => {
    const alias = apply({
      ...MIN,
      EDDY_MODEL_PROVIDER_TYPE: 'anthropic',
      EDDY_MODEL_MAX_OUTPUT_SIZE: '8192',
      EDDY_MODEL_REASONING_KEY: 'reasoning',
    }).models?.[ENV_MODEL_ALIAS_KEY];
    expect(alias?.maxOutputSize).toBe(8192);
    expect(alias?.reasoningKey).toBe('reasoning');
    expectConfigInvalid(() =>
      apply({ ...MIN, EDDY_MODEL_MAX_OUTPUT_SIZE: 'nope' }),
    );
  });
  ...
  it('maps the thinking variables', () => {
    const config = apply({
      ...MIN,
      EDDY_MODEL_DEFAULT_THINKING: 'true',
      EDDY_MODEL_THINKING_MODE: 'on',
      EDDY_MODEL_THINKING_EFFORT: 'high',
    });
    expect(config.defaultThinking).toBe(true);
    expect(config.thinking).toMatchObject({ mode: 'on', effort: 'high' });
    expect(apply({ ...MIN, EDDY_MODEL_DEFAULT_THINKING: '0' }).defaultThinking)
      .toBe(false);
  });

  it('rejects an invalid thinking mode', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, EDDY_MODEL_THINKING_MODE: 'bogus' }),
    );
  });

  it('maps EDDY_MODEL_ADAPTIVE_THINKING onto the alias', () => {
    expect(
      apply({ ...MIN, EDDY_MODEL_ADAPTIVE_THINKING: 'true' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(true);
    expect(
      apply({ ...MIN, EDDY_MODEL_ADAPTIVE_THINKING: 'false' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(false);
    expect(
  ...
  it('rejects an invalid EDDY_MODEL_ADAPTIVE_THINKING', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, EDDY_MODEL_ADAPTIVE_THINKING: 'maybe' }),
    );
  });
...
describe('loadRuntimeConfig vs readConfigFile (write-back isolation)', () => {
  it('injects the env model into runtime config but readConfigFile stays clean', () => {
    const dir = mkdtempSync(join(tmpdir(), 'eddy-env-model-'));
...
describe('stripEnvModelConfig (write-back guard)', () => {
  ...
  it('keeps user providers/models and a non-env default_model', () => {
    ...
    config.providers['eddy'] = { type: 'eddy', apiKey: 'k', baseUrl: 'https://x/v1' };
    config.providers[ENV_MODEL_PROVIDER_KEY] = { type: 'eddy', apiKey: 'env-key' };
    config.models = {
      'my-model': { provider: 'eddy', model: 'm', maxContextSize: 1000 },
    ...
    expect(stripped.providers['eddy']).toBeDefined();
...
describe('writeConfigFile never persists the env model', () => {
  it('strips env entries (incl. thinking) when a runtime config is written back', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'eddy-env-write-'));
    const path = join(dir, 'config.toml');
    writeFileSync(
      path,
  ...
  it('output never contains env-injected identifiers', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'eddy-env-write2-'));
    const path = join(dir, 'config.toml');
    writeFileSync(
      path,
      'default_model = "x"\n[providers.x]\ntype = "eddy"\napi_key = "k"\n[models.x]\nprovider = "x"\nmodel = "x"\nmax_context_size = 1000\n',
    ...
    try {
      const runtime = loadRuntimeConfig(path, {
        ...MIN,
        EDDY_MODEL_THINKING_EFFORT: 'low',
        EDDY_MODEL_DEFAULT_THINKING: 'true',
      });
      await writeConfigFile(path, runtime);
...
describe('EDDY_MODEL_DEFAULT_THINKING validation', () => {
  it('rejects a non-empty unparseable value', () => {
    expectConfigInvalid(() => apply({ ...MIN, EDDY_MODEL_DEFAULT_THINKING: 'flase' }));
  });

  it('accepts valid values and ignores when unset', () => {
    expect(apply({ ...MIN, EDDY_MODEL_DEFAULT_THINKING: 'true' }).defaultThinking).toBe(true);
    expect(apply({ ...MIN, EDDY_MODEL_DEFAULT_THINKING: '0' }).defaultThinking).toBe(false);
