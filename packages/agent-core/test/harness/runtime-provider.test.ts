...
import type { EddyConfig } from '../../src/config';
...
function resolveRuntimeProvider(input: {
  ...
  const manager = new ProviderManager({
    config: input.config,
    eddyRequestHeaders: input.eddyRequestHeaders,
...
const TEST_EDDY_HEADERS = {
  'User-Agent': 'eddy-code-cli/0.0.0-test',
  'X-Msh-Platform': 'eddy_code_cli',
...
describe('resolveRuntimeProvider model metadata', () => {
  it('uses config model metadata as the source of truth', () => {
    ...
    expect(resolved.provider.model).toBe('eddy-for-coding');
  });
  ...
  it('uses config Eddy capabilities without requiring an api key during OAuth setup', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
  ...
  it('does not infer Eddy capabilities from the provider model name', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        models: {
...
describe('resolveRuntimeProvider Eddy request headers', () => {
  it('does not set defaultHeaders when no eddyRequestHeaders or customHeaders exist', () => {
  ...
  it('uses only customHeaders when eddyRequestHeaders are missing', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
  ...
  it('passes eddyRequestHeaders through to Eddy provider defaultHeaders', () => {
    const resolved = resolveRuntimeProvider({
      config: BASE_CONFIG,
      eddyRequestHeaders: TEST_EDDY_HEADERS,
    });

    expect(resolved.provider).toMatchObject({
      type: 'eddy',
      defaultHeaders: TEST_EDDY_HEADERS,
    });
  });

  it('passes the prompt cache key to Eddy generation kwargs', () => {
    const resolved = resolveRuntimeProvider({
      config: BASE_CONFIG,
  ...
  it('lets provider customHeaders override eddyRequestHeaders', () => {
    const resolved = resolveRuntimeProvider({
      config: {
        ...BASE_CONFIG,
        providers: {
  ...
  it('does not apply eddyRequestHeaders to non-Eddy providers', () => {
    const resolved = resolveRuntimeProvider({
      config: {
...
describe('ProviderManager prompt cache key', () => {
  it('applies a prompt cache key to Eddy providers', () => {
    const manager = new ProviderManager({
      config: BASE_CONFIG,
      promptCacheKey: 'session-test',
    });
    const resolved = manager.resolveProviderConfig('eddy-code/eddy-for-coding');
  ...
  it('does not add generation kwargs to non-Eddy providers', () => {
    const manager = new ProviderManager({
      promptCacheKey: 'session-test',
      config: {
        defaultModel: 'gpt-alias',
  ...
  it('reads the current config when constructed with a function', () => {
    let sharedConfig: EddyConfig = { providers: {} };
    const manager = new ProviderManager({
      config: () => sharedConfig,
      promptCacheKey: 'session-test',
    });
