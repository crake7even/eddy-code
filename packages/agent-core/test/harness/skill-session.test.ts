...
describe('HarnessAPI session skills', () => {
  ...
  beforeEach(async () => {
    tmp = await mkdtemp(join(tmpdir(), 'eddy-core-skills-'));
  ...
  it('lists session skills without exposing content', async () => {
    ...
    expect(listed?.path.endsWith('/.eddy-code/skills/phase-one-review/SKILL.md')).toBe(true);
  ...
  it('resolves user skills from the OS home directory, not from the eddy home', async () => {
  ...
  it('resolves user skills from the OS home directory even when EDDY_CODE_HOME is set', async () => {
  ...
  it('expands skill body placeholders on user slash activation', async () => {
    ...
    const records = await readMainWire(created.sessionDir);
    const prompt = records.find((record) => record['type'] === 'turn.prompt');
    const skillDir = await realpath(join(workDir, '.eddy-code', 'skills', 'templated-review'));
  ...
  async function writeFlatSkill(name: string, lines: readonly string[]): Promise<void> {
    const dir = join(workDir, '.eddy-code', 'skills');
    await mkdir(dir, { recursive: true });
