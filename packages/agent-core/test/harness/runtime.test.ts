...
describe('EddyCore runtime config', () => {
  let tmp: string;
  ...
  it('uses the shared OAuth resolver for EddyCli service tokens', async () => {
    tmp = await mkdtemp(join(tmpdir(), 'eddy-core-runtime-'));
...
oauth = { storage = "file", key = "oauth/custom-eddy-code" }
...
`,
    ...
    expect(resolveOAuthTokenProvider).toHaveBeenCalledWith('managed:eddy-code', {
      storage: 'file',
      key: 'oauth/custom-eddy-code',
    });
    expect(session?.options.toolServices?.webSearcher).toBeDefined();

    await session!.options.toolServices?.webSearcher!.search('eddy');
...
`,
    ...
    const [coreRpc, sdkRpc] = createRPC<CoreAPI, SDKAPI>();
    const core = new EddyCore(coreRpc, { homeDir });
    const rpc = await sdkRpc({
      emitEvent: vi.fn(),
      requestApproval: vi.fn(async (): Promise<ApprovalResponse> => ({ decision: 'rejected' })),
      requestQuestion: vi.fn(async () => null),
