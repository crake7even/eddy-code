...
beforeEach(async () => {
  workDir = await mkdtemp(join(tmpdir(), 'eddy-agent-profile-'));
});
  ...
  it('loads YAML profiles, inherits templates, and renders with runtime context', async () => {
    const systemPath = await write(
      ...
      [
        'os={{ EDDY_OS }}',
        'cwd={{ EDDY_WORK_DIR }}',
        'listing={{ EDDY_WORK_DIR_LS }}',
        'agents={{ EDDY_AGENTS_MD }}',
        'skills={{ EDDY_SKILLS }}',
        'parent={{ parentOnly }}',
        'child={{ childOnly }}',
        'role={{ ROLE_ADDITIONAL }}',
        '{% if EDDY_OS == "macOS" %}nunjucks-ok{% endif %}',
      ].join('\n'),
    );
    await write(
      'agent.yaml',
...
describe('default agent profiles', () => {
  ...
  it('renders the bundled default prompt from the current runtime context', () => {
    ...
    expect(first).toContain('You are Eddy Code');
