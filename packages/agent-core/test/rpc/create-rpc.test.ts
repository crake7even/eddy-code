...
describe('createRPC', () => {
  ...
  it('binds prototype methods and rehydrates plain remote errors as EddyError(internal)', async () => {
    ...
    await expect(hostProxy.fail({ code: 'boom' })).rejects.toBeInstanceOf(EddyError);
    expect(hostImpl.approvals).toEqual(['approval-2']);
  });

  it('passes a thrown EddyError across the wire preserving code and details', async () => {
    ...
    await connectRemote({
      async prompt(payload) {
        if (payload.input === '') {
          throw new EddyError(ErrorCodes.REQUEST_PROMPT_INPUT_EMPTY, 'Prompt input cannot be empty', {
    ...
    expect(received).toBeInstanceOf(EddyError);
    const error = received as EddyError;
  ...
  it('serializes remote error details through JSON semantics before crossing the wire', async () => {
    ...
    await connectRemote({
      async fail() {
        ...
        throw new EddyError(ErrorCodes.INTERNAL, 'Remote failed', {
          details,
        });
      },
    });
  ...
  it('rehydrates provider.* codes as EddyError', async () => {
    interface RemoteSide {
    ...
    await connectRemote({
      async callProvider() {
        throw new EddyError(ErrorCodes.PROVIDER_RATE_LIMIT, 'Upstream rate limit', {
  ...
  it('collapses non-Error throws to EddyError(internal)', async () => {
    interface RemoteSide {
    ...
    await expect(remoteProxy.misbehave({})).rejects.toBeInstanceOf(EddyError);
  });
});
