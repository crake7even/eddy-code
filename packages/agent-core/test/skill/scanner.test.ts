...
describe('skill discovery', () => {
  it('resolves documented roots in precedence order with brand merging enabled by default', async () => {
    ...
    await mkdir(join(repoDir, '.eddy-code', 'skills'), { recursive: true });
    await mkdir(join(repoDir, '.agents', 'skills'), { recursive: true });
    await mkdir(join(homeDir, '.eddy-code', 'skills'), { recursive: true });
    ...
    expect(roots.map((root) => relative(realRepoDir, root.path))).toEqual([
      '.eddy-code/skills',
      '.agents/skills',
      relative(realRepoDir, await realpath(join(homeDir, '.eddy-code', 'skills'))),
  ...
  it('uses only the first brand directory when brand merging is disabled', async () => {
    ...
    expect(roots.map((root) => root.path)).toEqual([
      await realpath(join(repoDir, '.eddy-code', 'skills')),
      await realpath(join(homeDir, '.eddy-code', 'skills')),
    ]);
  });
  ...
  it('discovers flat markdown skills, keeps directory skills over same-name flat files, and preserves source precedence', async () => {
    ...
    const projectRoot = join(repoDir, '.eddy-code', 'skills');
    const userRoot = join(homeDir, '.eddy-code', 'skills');
...
describe('discoverSkills shape and ordering', () => {
  it('parses frontmatter name/description for subdir skills', async () => {
    const { repoDir } = await makeWorkspace();
    const root = join(repoDir, '.eddy-code', 'skills');
...
describe('resolveSkillRoots ordering and priority', () => {
  ...
  it('prefers brand over generic user dirs even when both contain skills with the same name', async () => {
    ...
    const brand = join(homeDir, '.eddy-code', 'skills');
  ...
  it('returns proj-brand, proj-generic, user-brand, user-generic, builtin in that order without merging', async () => {
    const { homeDir, repoDir, workDir } = await makeWorkspace();
    const userBrand = join(homeDir, '.eddy-code', 'skills');
    const userGeneric = join(homeDir, '.agents', 'skills');
    const projBrand = join(repoDir, '.eddy-code', 'skills');
  ...
  it('defaults to merging user brand dirs', async () => {
    ...
    expect(paths).toContain(await realpath(join(homeDir, '.eddy-code', 'skills')));
  });
});
...
describe('resolveSkillRoots extra dirs', () => {
  ...
  it('combines explicit dirs with extra dirs and suppresses auto-discovery', async () => {
    ...
    const projectBrand = join(repoDir, '.eddy-code', 'skills');
...
async function makeWorkspace(): Promise<{
  ...
  const tmp = await mkdtemp(join(tmpdir(), 'eddy-skill-scanner-'));
  tempDirs.push(tmp);
...
describe('project root discovery (.git walk-up)', () => {
  it('walks up to the nearest .git ancestor for project-scope discovery', async () => {
    ...
    const repo = await mkdtemp(join(tmpdir(), 'eddy-skill-walkup-'));
    tempDirs.push(repo);
    await mkdir(join(repo, '.git'), { recursive: true });
    const repoEddy = join(repo, '.eddy-code', 'skills');
    await writeSkill(repoEddy, join('foo', 'SKILL.md'), [
      '---',
      'name: foo',
    ...
    expect(projectPaths).toContain(await realpath(repoEddy));
  });

  it('falls back to the work dir when no .git marker is found anywhere up the chain', async () => {
    const { homeDir } = await makeWorkspace();
    const noGitTmp = await mkdtemp(join(tmpdir(), 'eddy-skill-nogit-'));
    tempDirs.push(noGitTmp);
    const project = join(noGitTmp, 'project');
    await mkdir(join(project, '.eddy-code', 'skills'), { recursive: true });
    ...
    expect(projectPaths.some((p) => p.includes(join('project', '.eddy-code', 'skills')))).toBe(false);
  });
});
