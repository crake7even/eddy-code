...
export const EDDY_CODE_PLATFORM = 'eddy_code_cli';
...
export interface EddyHostIdentity {
  readonly userAgentProduct: string;
  readonly version: string;
  readonly userAgentSuffix?: string | undefined;
}

export interface EddyIdentityOptions extends EddyHostIdentity {
  readonly homeDir: string;
}

export interface CreateEddyDeviceIdOptions {}

export function createEddyDeviceId(
  homeDir: string,
  _options: CreateEddyDeviceIdOptions = {},
): string {
...
export function createEddyDeviceHeaders(options: {
  readonly homeDir: string;
  readonly version: string;
}): DeviceHeaders {
  return {
    'X-Msh-Platform': EDDY_CODE_PLATFORM,
    'X-Msh-Version': requiredAsciiHeader(options.version, 'Eddy identity version'),
    'X-Msh-Device-Name': asciiHeader(hostname()),
    'X-Msh-Device-Model': asciiHeader(deviceModel()),
    'X-Msh-Os-Version': asciiHeader(release()),
    'X-Msh-Device-Id': createEddyDeviceId(options.homeDir),
  };
}

export function createEddyUserAgent(options: {
  readonly userAgentProduct: string;
  readonly version: string;
  readonly userAgentSuffix?: string | undefined;
}): string {
  const product = requiredAsciiHeader(options.userAgentProduct, 'Eddy identity product');
  const version = requiredAsciiHeader(options.version, 'Eddy identity version');
  const suffix =
...
export function createEddyDefaultHeaders(options: EddyIdentityOptions): Record<string, string> {
  return {
    'User-Agent': createEddyUserAgent(options),
    ...createEddyDeviceHeaders({
...
export function assertEddyHostIdentity(identity: EddyHostIdentity | undefined): EddyHostIdentity {
  if (identity === undefined) {
    throw new Error('Eddy host identity is required. Pass the host product name and version.');
  }
  requiredAsciiHeader(identity.userAgentProduct, 'Eddy identity product');
  requiredAsciiHeader(identity.version, 'Eddy identity version');
  return identity;
}
