...
import { EDDY_CODE_PLATFORM } from '@eddy-code/config';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'eddy-sdk-prompt-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('Session.prompt events', () => {
  ...
  it('emits mapped turn events through Session.onEvent', async () => {
    ...
    try {
      ...
      expect(fakeProviderState.calls[0]?.systemPrompt).toContain('You are Eddy Code');
...
async function configureFakeProvider(harness: InstanceType<typeof EddyHarness>): Promise<void> {
