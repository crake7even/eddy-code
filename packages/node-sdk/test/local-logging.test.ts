...
import { EddyHarness, log } from '#/index';
...
const LOG_ENV_KEYS = [
  'EDDY_LOG_LEVEL',
  'EDDY_LOG_GLOBAL_MAX_BYTES',
  'EDDY_LOG_GLOBAL_FILES',
  'EDDY_LOG_SESSION_MAX_BYTES',
  'EDDY_LOG_SESSION_FILES',
] as const;
...
describe('Local logging — harness integration', () => {
  it('writes session-tagged entries to session log only and untagged entries to global', async () => {
    const homeDir = await makeTempDir('eddy-log-home-');
    const workDir = await makeTempDir('eddy-log-work-');

    const harness = new EddyHarness({ identity: TEST_IDENTITY, homeDir });
    ...
    const globalPath = join(homeDir, 'logs', 'eddy-code.log');
    const sessionLogPath = join(summary.sessionDir, 'logs', 'eddy-code.log');

    // Trigger an export — this flushes both global and session via EddyCore
  ...
  it('default export bundles session log only; no globalLogPath in manifest', async () => {
    ...
    expect(entries.has('logs/eddy-code.log')).toBe(true);
    expect(entries.has('logs/global/eddy-code.log')).toBe(false);
    expect(entries.get('logs/eddy-code.log')!.toString('utf-8')).toContain(
      'session export marker',
    );
    expect(result.manifest.sessionLogPath).toBe('logs/eddy-code.log');
    ...
    const manifest = JSON.parse(entries.get('manifest.json')!.toString('utf-8')) as Record<
      ...
      unknown
    >;
    expect(manifest['sessionLogPath']).toBe('logs/eddy-code.log');
  ...
  it('default export works when no session log file exists', async () => {
    ...
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/eddy-code.log')).toBe(false);
  ...
  it('default export includes rotated session log files without requiring active eddy-code.log', async () => {
    const env = snapshotLogEnv();
    process.env['EDDY_LOG_LEVEL'] = 'warn';
    process.env['EDDY_LOG_SESSION_MAX_BYTES'] = '1024';
    process.env['EDDY_LOG_SESSION_FILES'] = '2';
    try {
      const homeDir = await makeTempDir('eddy-log-home-');
      const workDir = await makeTempDir('eddy-log-work-');
      const harness = new EddyHarness({ identity: TEST_IDENTITY, homeDir });
      ...
      const sessionLogEntries = [...entries.keys()].filter(
        (entry) => entry === 'logs/eddy-code.log' || entry.startsWith('logs/eddy-code.log.'),
      );
      expect(sessionLogEntries.length).toBeGreaterThan(0);
      expect(sessionLogEntries).toContain('logs/eddy-code.log.1');
      expect(entries.has('logs/global/eddy-code.log')).toBe(false);
      if (entries.has('logs/eddy-code.log')) {
        expect(result.manifest.sessionLogPath).toBe('logs/eddy-code.log');
      } else {
        expect(result.manifest.sessionLogPath).toBeUndefined();
      }
  ...
  it('--include-global-log bundles global active and sets manifest field', async () => {
    ...
    const entries = readZipEntries(zipBuf);
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/global/eddy-code.log')).toBe(true);
    expect(result.manifest.globalLogPath).toBe('logs/global/eddy-code.log');
    // Global log carries entries that don't have a sessionId routed to a sink.
    expect(entries.get('logs/global/eddy-code.log')!.toString('utf-8')).toContain(
      'untagged probe',
    );
  });

  it('--include-global-log bundles the active root global log path', async () => {
    const firstHome = await makeTempDir('eddy-log-home-a-');
    const secondHome = await makeTempDir('eddy-log-home-b-');
    const workDir = await makeTempDir('eddy-log-work-');
    const first = new EddyHarness({ identity: TEST_IDENTITY, homeDir: firstHome });
    const firstSession = await first.createSession({ id: 'ses_first_global_export', workDir });
    const second = new EddyHarness({ identity: TEST_IDENTITY, homeDir: secondHome });
    ...
    const entries = readZipEntries(await readFile(result.zipPath));
    const globalLog = entries.get('logs/global/eddy-code.log')!.toString('utf-8');
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'eddy-code.log'));
  ...
  it('logs export flush failures without failing the export', async () => {
    ...
    try {
      ...
      const entries = readZipEntries(await readFile(result.zipPath));
      const sessionLog = entries.get('logs/eddy-code.log')!.toString('utf-8');
      const globalLog = entries.get('logs/global/eddy-code.log')!.toString('utf-8');
  ...
  it('multiple EddyHarness constructions in the same process do not throw', async () => {
    const homeDir = await makeTempDir('eddy-log-home-');
    expect(() => new EddyHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => new EddyHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
    expect(() => new EddyHarness({ identity: TEST_IDENTITY, homeDir })).not.toThrow();
  });
  ...
  it('uses the latest harness homeDir for global diagnostic logging', async () => {
    ...
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'eddy-code.log'));
    const secondLog = await readFile(join(secondHome, 'logs', 'eddy-code.log'), 'utf-8');
  ...
  it('checks that an empty session log directory does not get a log file', async () => {
    ...
    const env = snapshotLogEnv();
    process.env['EDDY_LOG_LEVEL'] = 'off';
    try {
      const homeDir = await makeTempDir('eddy-log-home-');
      const workDir = await makeTempDir('eddy-log-work-');
      const harness = new EddyHarness({ identity: TEST_IDENTITY, homeDir });
      ...
      expect(logsDir).not.toContain('eddy-code.log');
    } finally {
      restoreLogEnv(env);
    }
  });

  it('EddyHarness.close() flushes the global log', async () => {
    const homeDir = await makeTempDir('eddy-log-home-');
    const harness = new EddyHarness({ identity: TEST_IDENTITY, homeDir });
