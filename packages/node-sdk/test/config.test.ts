...
import { EddyError, EddyHarness } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'eddy-sdk-config-'));
  tempDirs.push(dir);
  return dir;
}

const COMPLETE_TOML = `
default_model = "eddy-for-coding"
...
describe('SDK config TOML', () => {
  it('parses the documented config shape and keeps TUI-only fields in raw', () => {
    ...
    expect(config.defaultModel).toBe('eddy-for-coding');
    ...
    const provider = config.providers['eddy-for-coding'];
    ...
    expect(config.models?.['eddy-for-coding']).toMatchObject({
      provider: 'eddy-for-coding',
      model: 'eddy-for-coding',
  ...
  it('writes typed fields in snake_case and preserves unknown raw sections', async () => {
    ...
    expect(text).toContain('default_model = "eddy-for-coding"');
    ...
    expect(text).toContain('display_name = "Eddy for Coding"');
...
`);
    ...
    expect(config.services?.eddyCliSearch).toMatchObject({
      baseUrl: 'https://example.test/search',
      apiKey: 'sk-search',
    });
...
describe('EddyHarness config API', () => {
  it('loads default config when missing and deep-merges setConfig patches from disk', async () => {
    ...
    expect(config.providers['eddy-for-coding']).toMatchObject({
      type: 'eddy',
      baseUrl: 'https://api.eddy-code.local/coding/v1',
      apiKey: 'sk-updated',
      env: { GOOGLE_CLOUD_PROJECT: 'project-1' },
    });
    expect(config.services?.eddyCliSearch?.apiKey).toBe('sk-search-updated');
    expect(config.raw?.['theme']).toBe('dark');
  ...
  it('does not write invalid config patches', async () => {
    ...
    await expect(setInvalidConfig).rejects.toBeInstanceOf(EddyError);
