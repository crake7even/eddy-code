...
import { EddyHarness, type Event, type EddyError } from '#/index';
...
describe('Session plan, compact, usage, and resume APIs', () => {
  it('sets plan mode through manualEnterPlan and clears the active plan file', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-plan-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-plan-work-');
    await writeTestConfig(homeDir);
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('prepares the plans directory without creating plan files on repeated toggles', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-plan-toggle-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-plan-toggle-work-');
    await writeTestConfig(homeDir);
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects manual compaction on an empty session with compaction.unable', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-compact-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-compact-work-');
    await writeTestConfig(homeDir);
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('returns current session usage totals', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-usage-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-usage-work-');
    await writeTestConfig(homeDir);
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('resumes a persisted session and restores runtime plan mode from wire history', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-resume-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-resume-work-');
    await writeTestConfig(homeDir);
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it.todo('marks resumed plan mode active when the restored plan has no plan data', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-resume-legacy-plan-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-resume-legacy-plan-work-');
    await writeTestConfig(homeDir);
    const createdHarness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
    let sessionId = '';
    let sessionDir = '';
    ...
    const resumedHarness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
    try {
  ...
  it('forks a session and returns an active fork session', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-fork-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-fork-work-');
    await writeTestConfig(homeDir);
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
  ...
  it('rejects an empty resume id', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-resume-empty-home-');
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
