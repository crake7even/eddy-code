...
import { EddyError, EddyHarness, type Event } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'eddy-sdk-rename-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('EddyHarness.renameSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    try {
      ...
      await expect(missingRename).rejects.toBeInstanceOf(EddyError);
