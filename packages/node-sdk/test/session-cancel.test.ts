...
import type { EddyError, Event } from '#/index';
...
const { EddyHarness } = await import('#/index');
...
describe('Session.cancel', () => {
  it('cancels an active streaming turn and emits turn_ended(cancelled)', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-cancel-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-cancel-work-');
  ...
  it('rejects manual compaction on an empty session with compaction.unable', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-cancel-compact-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-cancel-compact-work-');
    await writeFakeModelConfig(homeDir);
    const harness = new EddyHarness({ homeDir, identity: TEST_IDENTITY });
