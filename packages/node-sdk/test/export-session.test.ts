...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'eddy-sdk-export-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('exportSessionDirectory', () => {
  ...
  it('omits global log manifest path when the global log cannot be bundled', async () => {
    ...
    expect(result.entries).not.toContain('logs/global/eddy-code.log');
    const entries = readZipEntries(await readFile(outputPath));
...
describe('EddyHarness.exportSession', () => {
  ...
  it('rejects missing session ids', async () => {
    ...
    await expect(missingExport).rejects.toBeInstanceOf(EddyError);
