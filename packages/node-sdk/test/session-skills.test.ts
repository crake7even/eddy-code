...
import type { Event, EddyError, SkillActivatedEvent, SkillSummary } from '#/index';
...
vi.mock('@eddy-code/ltod', async (importOriginal) => {
...
const { EddyHarness, Session } = await import('#/index');
...
describe('Session skills', () => {
  it('lists session skills without exposing content', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-skills-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-skills-work-');
    ...
    try {
      ...
      expect(listed?.path.endsWith('/.eddy-code/skills/review/SKILL.md')).toBe(true);
  ...
  it('resolves user skills from the OS home directory, independently of EDDY_CODE_HOME', async () => {
    const homeDir = await makeTempDir(tempDirs, 'eddy-sdk-skills-home-');
    const processHome = await makeTempDir(tempDirs, 'eddy-sdk-skills-process-home-');
    const workDir = await makeTempDir(tempDirs, 'eddy-sdk-skills-work-');
    vi.stubEnv('HOME', processHome);
    vi.stubEnv('EDDY_CODE_HOME', homeDir);
    ...
    const harness = new EddyHarness({ identity: TEST_IDENTITY });
