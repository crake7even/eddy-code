...
import { EddyHarness } from '#/index';
import type { EddyError } from '#/index';
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'eddy-sdk-create-'));
  tempDirs.push(dir);
  return dir;
}

async function writeTestModelConfig(homeDir: string, modelName = 'eddy-test-model'): Promise<void> {
  await writeFile(
...
describe('EddyHarness.createSession transport link', () => {
...
function coreSessionIds(harness: EddyHarness): readonly string[] {
  const core = (
