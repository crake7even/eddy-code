...
import type { EddyConfig, ModelAlias } from '@eddy-code/agent-core';
import {
  catalogBaseUrl,
...
/** Path to the local catalog cache inside the eddy home directory. */
export function catalogCachePath(eddyHome: string): string {
  return join(eddyHome, 'catalog-cache.json');
}
 ...
 */
export function saveCatalogCache(catalog: Catalog, eddyHome: string): void {
  try {
    const path = catalogCachePath(eddyHome);
 ...
 */
export function loadCatalogCache(eddyHome: string): Catalog | undefined {
  try {
    const raw = readFileSync(catalogCachePath(eddyHome), 'utf-8');
...
/** Builds a eddy-code model alias from a normalized catalog model. */
 ...
 * `__EDDY_CODE_BUILT_IN_CATALOG__` constant injected by tsdown at build
