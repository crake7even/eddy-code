...
import { assertEddyHostIdentity } from '@eddy-code/config';

import { EddyAuthFacade } from '#/auth';
...
export class EddyHarness {
  readonly homeDir: string;
  readonly configPath: string;
  readonly auth: EddyAuthFacade;

  private readonly identity: EddyHostIdentity | undefined;
  private readonly uiMode: string;
  private readonly activeSessions = new Map<string, Session>();
  private readonly rpc: SDKRpcClient;

  constructor(options: EddyHarnessOptions) {
    this.identity =
      options.identity === undefined ? undefined : assertEddyHostIdentity(options.identity);
  ...
  async getConfig(options: GetConfigOptions = {}): Promise<EddyConfig> {
  ...
  async setConfig(patch: EddyConfigPatch): Promise<EddyConfig> {
...
function normalizeSessionId(value: string): string {
  if (typeof value !== 'string') {
    throw new EddyError(ErrorCodes.SESSION_ID_REQUIRED, 'Session id is required.');
  }
  const normalized = value.trim();
  if (normalized.length === 0) {
    throw new EddyError(ErrorCodes.SESSION_ID_EMPTY, 'Session id cannot be empty.');
  }
  return normalized;
}
