import { ErrorCodes, EddyError, type AgentContextData, type EddyErrorCode } from '@eddy-code/agent-core';
...
export class Session {
  ...
  private ensureOpen(): void {
    if (this.closed) {
      throw new EddyError(ErrorCodes.SESSION_CLOSED, 'Session is closed');
    }
  }
}
...
function normalizeRequiredString(
  ...
  if (normalized.length === 0) {
    throw new EddyError(code, message);
  }
  return normalized;
}
