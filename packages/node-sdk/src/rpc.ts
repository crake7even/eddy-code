...
import { createEddyDefaultHeaders } from '@eddy-code/config';
...
export interface SDKRpcClientOptions {
  ...
  readonly identity?: EddyHostIdentity | undefined;
...
export class SDKRpcClient {
  ...
  constructor(options: SDKRpcClientOptions = {}) {
    ...
    const homeDir = resolveEddyHome(options.homeDir);
    const eddyRequestHeaders =
      options.identity === undefined
        ? undefined
        : createEddyDefaultHeaders({ homeDir, ...options.identity });
    this.core = new EddyCore(coreRpc, {
  ...
  async getConfig(input?: GetConfigOptions): Promise<EddyConfig> {
    const rpc = await this.getRpc();
    return rpc.getEddyConfig(input ?? {});
  }
  ...
  async setConfig(input: EddyConfigPatch): Promise<EddyConfig> {
    const rpc = await this.getRpc();
    return rpc.setEddyConfig(input);
  }

  async removeProvider(providerId: string): Promise<EddyConfig> {
    const rpc = await this.getRpc();
    return rpc.removeEddyProvider({ providerId });
  }
  ...
  async createGoal(input: CreateGoalRpcInput): Promise<import('@eddy-code/agent-core').GoalSnapshotData> {
    const rpc = await this.getRpc();
  ...
  async updateGoalStatus(input: UpdateGoalStatusRpcInput): Promise<import('@eddy-code/agent-core').GoalSnapshotData | null> {
    const rpc = await this.getRpc();
  ...
  async cancelGoal(input: SessionIdRpcInput): Promise<import('@eddy-code/agent-core').GoalSnapshotData | null> {
    const rpc = await this.getRpc();
  ...
  async getGoal(input: SessionIdRpcInput): Promise<import('@eddy-code/agent-core').GetGoalResult> {
    const rpc = await this.getRpc();
    return rpc.getGoal({
      sessionId: input.sessionId,
      agentId: this.interactiveAgentId,
  ...
  async setGoalBudget(input: SetGoalBudgetRpcInput): Promise<import('@eddy-code/agent-core').GoalSnapshotData> {
    const rpc = await this.getRpc();
