...
import { EddyFiles } from '#/providers/eddy-files';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

function createProvider(): EddyChatProvider {
...
describe('EddyFiles', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'eddy-files-test-'));
  });
  ...
  describe('exposure on EddyChatProvider', () => {
    it('exposes files property returning a EddyFiles instance', () => {
      const provider = createProvider();
      const files = provider.files;
      expect(files).toBeInstanceOf(EddyFiles);
