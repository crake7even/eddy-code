...
describe('integration: streaming provider contracts', () => {
  describe('synthetic Eddy-style SSE streaming', () => {
    it('multi-chunk text + tool_call + tool_call_part + usage', async () => {
      ...
      const stream = buildStream(parts, { id: 'eddy-resp-1', usage });
      const provider = new SingleStreamProvider(stream, 'eddy');
      ...
      expect(result.id).toBe('eddy-resp-1');
    });
    ...
    it('reasoning_content chunks interleave with text', async () => {
      ...
      const stream = buildStream(parts, { id: 'eddy-think-1' });
      const provider = new SingleStreamProvider(stream, 'eddy');
