...
function createProvider(baseUrl: string): EddyChatProvider {
...
describe('e2e: eddy adapter', () => {
  it('sends EddyCli chat-completion requests, streams reasoning + tool-call deltas, and preserves usage', async () => {
    await withHarness(async (harness) => {
      ...
      expect(result.id).toBe('chatcmpl-eddy-1');
  ...
  it('propagates provider HTTP failures as APIStatusError', async () => {
    await withHarness(async (harness) => {
      let capturedRequest: Record<string, unknown> | null = null;
