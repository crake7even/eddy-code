...
function createProvider(stream: boolean = false): EddyChatProvider {
  return new EddyChatProvider({
    model: 'eddy-k2-turbo-preview',
    apiKey: 'test-key',
    stream,
  });
}

type EddyGenerationState = {
...
function getGenerationState(provider: EddyChatProvider): EddyGenerationState {
  return Reflect.get(provider, '_generationKwargs') as EddyGenerationState;
}

/** Capture the request body sent to OpenAI by mocking the client. */
async function captureRequestBody(
  provider: EddyChatProvider,
  systemPrompt: string,
  tools: Tool[],
  history: Message[],
  ...
  (provider as any)._client.chat.completions.create = vi
    ...
    .mockImplementation((params: unknown) => {
      ...
      return Promise.resolve(makeChatCompletionResponse('eddy-k2'));
    });
...
describe('EddyChatProvider', () => {
  describe('message conversion', () => {
    ...
    it('adds Eddy-only types to JetBrains-like enum-only tool parameters without mutation', async () => {
      const provider = createProvider();
    ...
    it('dereferences draft-7 definitions and normalizes referenced enum-only schemas for Eddy', async () => {
      const provider = createProvider();
    ...
    it('assistant with reasoning content', async () => {
      ...
      // Snapshot of the expected wire format.
      // Eddy injects ThinkPart as `reasoning_content` field on the
      // assistant message (EddyCli API extension).
      expect(body['messages']).toEqual([
        { role: 'user', content: 'What is 2+2?' },
        {
          role: 'assistant',
  ...
  describe('generation kwargs', () => {
    ...
    it('sends no completion-token cap by default 鈥?provider is responsible for clamping', async () => {
      const provider = createProvider();
      const history: Message[] = [
        { role: 'user', content: [{ type: 'text', text: 'Hi' }], toolCalls: [] },
      ];
    ...
    it('passes constructor generation kwargs into the request body', async () => {
      const provider = new EddyChatProvider({
        model: 'eddy-k2-turbo-preview',
        apiKey: 'test-key',
        stream: false,
  ...
  describe('provider properties', () => {
    it('has correct name and model', () => {
      const provider = createProvider();
      expect(provider.name).toBe('eddy');
      expect(provider.modelName).toBe('eddy-k2-turbo-preview');
    });

    it('throws during generation when no constructor or request API key is provided', async () => {
      // Save and clear env var
      const saved = process.env['EDDY_API_KEY'];
      delete process.env['EDDY_API_KEY'];
      try {
        const provider = new EddyChatProvider({ model: 'test' });
        await expect(provider.generate('', [], [])).rejects.toThrow(/options\.auth\.apiKey/);
      } finally {
        if (saved !== undefined) {
          process.env['EDDY_API_KEY'] = saved;
        }
      }
    });
    ...
    it('passes request-scoped auth to the client factory', async () => {
      ...
      const client = {
        chat: {
          completions: {
            create: vi.fn().mockResolvedValue(makeChatCompletionResponse('eddy-k2')),
          },
        },
      };
      const provider = new EddyChatProvider({
    ...
    it('withThinking returns a new instance', () => {
      ...
      expect(newProvider).toBeInstanceOf(EddyChatProvider);
  ...
  describe('clone client sharing', () => {
    ...
    function getInternalClient(provider: EddyChatProvider): unknown {
  ...
  describe('streaming tool call routing', () => {
    ...
    it('handles non-indexed tool_call delta with arguments only (emit ToolCallPart)', async () => {
      ...
      // A pure arguments-only delta without an index 鈥?no concrete name yet.
...
describe('extractUsageFromChunk', () => {
  ...
  it('extracts choices[0].usage (EddyCli proprietary)', () => {
    const chunk = {
...
describe('extractUsage', () => {
  ...
  it('extracts usage with EddyCli cached_tokens', () => {
    const usage = extractUsage({
      prompt_tokens: 100,
