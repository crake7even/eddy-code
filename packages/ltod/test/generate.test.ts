...
describe('generate()', () => {
  ...
  it('calls onMessagePart and onToolCall callbacks correctly', async () => {
    ...
    // Callback parts should be copies 鈥?mutating pendingPart should not affect them.
  ...
  it('handles indexed deltas arriving after the pending tool call has changed', async () => {
    ...
    const stream = createMockStream([
      ...
      // Late delta for tc-0 鈥?must be routed via the index map.
      { type: 'tool_call_part', argumentsPart: '}', index: 0 },
    ]);
    const provider = createMockProvider(stream);
  ...
  //
  //   tc0-header 鈫?tc1-header 鈫?tc0-args 鈫?tc1-args 鈫?done
  //
    ...
    const result = await generate(provider, '', [], [], {
      async onToolCall(tc: ToolCall): Promise<void> {
        ...
        if (msg === null) {
          ...
          // JSON 鈥?verify that directly.
          observations.push({
            id: tc.id,
            totalToolCalls: -1, // unknown 鈥?message ref not yet bound
