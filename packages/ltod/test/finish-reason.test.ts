...
describe('normalizeOpenAIFinishReason (Eddy + OpenAILegacy shared helper)', () => {
  // Covers A for Eddy + OpenAILegacy.
...
function makeEddyStream(rawFinish: string | null | undefined): AsyncIterable<unknown> {
  // Eddy / Chat Completions stream: emit one content chunk + a terminal
...
function createEddyProvider(response: unknown, stream: boolean): EddyChatProvider {
...
describe('EddyChatProvider finish reason (stream, table coverage)', () => {
  // A + B coverage for Eddy.
  it.each<[string, FinishReason, string]>([
    ...
    async (raw, expectedFinish, expectedRaw) => {
      const provider = createEddyProvider(makeEddyStream(raw), true);
  ...
  it('returns null finishReason when stream never emits finish_reason', async () => {
    ...
    const provider = createEddyProvider(makeAsyncIterable(chunks), true);
  ...
  it('captures finish_reason from a non-stream response', async () => {
    const provider = createEddyProvider(
      {
        id: 'chatcmpl-ns',
        choices: [
          {
  ...
  // D (non-stream) coverage for Eddy.
...
describe('OpenAILegacyChatProvider finish reason (stream + non-stream)', () => {
  ...
  it.each<[string, FinishReason, string]>([
    ...
    'raw stream finish_reason %j maps to %j (raw=%j)',
    async (raw, expectedFinish, expectedRaw) => {
      const provider = createOpenAILegacyProvider(makeEddyStream(raw), true);
...
// =====================================================================
// OpenAI Responses API 鈥?stream event-based capture (A + B + C + D)
// =====================================================================
...
describe('OpenAIResponsesStreamedMessage finish reason', () => {
  ...
  // B: last chunk carries finish reason 鈥?already exercised above, but
...
// =====================================================================
// Anthropic 鈥?stream message_delta capture (A + B + C + D)
// =====================================================================
...
// =====================================================================
// Google GenAI 鈥?normalization table + stream + fallback (A-D)
// =====================================================================
