...
describe('derefJsonSchema', () => {
  ...
  it('preserves $defs when cyclic refs remain unresolved', () => {
    // A references B, B references A 鈥?classic cycle. resolveNode() leaves
    ...
    // Walk the result and confirm at least one remaining $ref points at $defs 鈥?
...
describe('normalizeEddyToolSchema', () => {
  it.each([
    {
      name: 'string enum',
    ...
    ({ property, expectedType }) => {
      ...
      const result = normalizeEddyToolSchema(schema);
  ...
  it('fails fast for mixed enum types instead of emitting an unsupported Eddy type array', () => {
    const schema = {
      type: 'object',
      properties: {
    ...
    expect(() => normalizeEddyToolSchema(schema)).toThrow(
  ...
  it('does not default the root schema itself to string', () => {
    expect(normalizeEddyToolSchema({})).toEqual({});
  });
