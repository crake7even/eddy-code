 ...
 * `xhigh` ceiling; Eddy and Gemini cap `xhigh`/`max` at `high`; Anthropic
 ...
 *   Completions鈥搒tyle providers and Anthropic surface a dedicated value.
 ...
 *
 * Each provider implementation (Eddy, OpenAI, Anthropic, Google GenAI, etc.)
...
export interface ChatProvider {
  /** Short identifier for the provider backend (e.g. `"eddy"`, `"anthropic"`). */
  readonly name: string;
  /** Model name passed to the provider API (e.g. `"eddy-code-v1-auto"`). */
   ...
   * returned clone 鈥?the clone is expected to share transport state with the
   * original. See `EddyChatProvider._clone()` for the rationale.
   */
