...
export function extractUsage(usage: unknown): TokenUsage | null {
  ...
  let cached = 0;
  // EddyCli proprietary: top-level cached_tokens
 ...
 *
 * Used by both the Eddy and OpenAI Legacy adapters because they share the
 * Chat Completions wire format. Returns `{ finishReason: null,
 * rawFinishReason: null }` when the provider value is missing or `null` so
