...
export interface EddyUploadOptions {
  auth?: ProviderRequestAuth;
  signal?: AbortSignal;
}

export interface EddyFilesOptions {
  apiKey?: string;
  baseUrl?: string;
...
/**
 * Eddy-specific file upload client.
 *
 * Wraps the underlying OpenAI-compatible `files.create` API to upload videos
 * to EddyCli's file service and return them as {@link VideoURLPart} values
 * suitable for use in chat messages.
 *
 * A `EddyFiles` instance is typically obtained from
 * {@link EddyChatProvider.files}.
 */
export class EddyFiles {
  ...
  constructor(options: EddyFilesOptions) {
    ...
    if (typeof input === 'string') {
      ...
      if (mimeType === undefined || !mimeType.startsWith('video/')) {
        throw new ChatProviderError(
          `EddyFiles.uploadVideo: file extension does not indicate a video type: ${filename}`,
        );
      }
    ...
    return resolveAuthBackedClient(
      ...
      (a) => {
        ...
        return new OpenAIClient({
          apiKey: requireProviderApiKey('EddyFiles.uploadVideo', a, this._apiKey),
