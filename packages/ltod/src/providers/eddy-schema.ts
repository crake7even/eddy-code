...
// Child-schema positions that this Eddy normalizer knows how to walk. This is
// also the source of truth for child-schema keywords that imply the parent
// schema's type. It is not a list of keywords that EddyCli accepts on the wire.
 ...
 * Eddy tool compatibility.
 *
 * EddyCli's tool validator rejects some valid JSON Schema shapes when nested
 ...
 */
export function normalizeEddyToolSchema(schema: Record<string, unknown>): Record<string, unknown> {
  return ensureEddyPropertyTypes(derefJsonSchema(schema));
}

function ensureEddyPropertyTypes(schema: Record<string, unknown>): Record<string, unknown> {
...
function inferTypeFromValues(values: unknown[]): JsonSchemaType {
  ...
  throw new Error('Mixed JSON Schema enum or const types are not supported by Eddy tool schemas.');
}
