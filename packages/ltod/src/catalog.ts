...
export interface CatalogProviderEntry {
  ...
  readonly api?: string;
  /** Env var names carrying credentials 鈥?surfaced as a hint by callers. */
 ...
 * appends `/v1/messages` itself 鈥?so a catalog `api` ending in `/v1` would POST
...
function catalogReasoningKey(interleaved: CatalogModelEntry['interleaved']): string | undefined {
  // models.dev allows `interleaved: true` as "general support" 鈥?read it as
