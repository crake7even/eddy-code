...
| --- | --- | --- |
| `eddy-code` | `apps/eddy-code` | CLI / TUI application — provides the `eddy` command after install |
| `@eddy-code/sdk` | `packages/node-sdk` | Public TypeScript SDK |
...
- `@eddy-code/agent-core`
- `@eddy-code/oauth`
- `@eddy-code/jian`
- `@eddy-code/ltod`
- `@eddy-code/vis`
- `@eddy-code/vis-server`
- `@eddy-code/vis-web`

Version impact from internal dependencies must be judged manually. The published artifacts for CLI and SDK bundle internal workspace packages into the artifact itself; runtime `dependencies` of published packages must not include any `@eddy-code/*` internal workspace packages.
...
| --- | --- |
| Only modifies TUI behavior in `eddy-code` | Add `patch` / `minor` / `major` to `eddy-code` |
| Only modifies internal packages, no user-visible change in SDK / CLI | Usually no changeset needed |
| Internal package fix changes the CLI user experience | Add a changeset to `eddy-code` describing the user-visible fix |
| Internal package adds a new capability exposed by the SDK | Add a changeset to `@eddy-code/sdk` |
| SDK behavior change affects CLI user experience | Add changesets to both `@eddy-code/sdk` and `eddy-code` |
| Provider abstraction change affects SDK / CLI | Add changesets to the affected `@eddy-code/sdk` and/or `eddy-code` |
...
1. Open each publishable package's page on the npm website, e.g. `https://www.npmjs.com/package/eddy-code`.
...
| Field | Value |
| --- | --- |
| GitHub Organization | `crake7even` |
| GitHub Repository | `eddy-code` |
...
- Do not add release changesets for private internal packages; only select `eddy-code` and `@eddy-code/sdk`.
- If a change in an underlying internal package alters user-visible behavior or public API of a publishable package, add a changeset to the affected publishable package. For example, when a bug fixed in `@eddy-code/agent-core` resolves an issue CLI users encounter, add a changeset to `eddy-code` describing the user-visible fix.
- `eddy-code` is the official CLI package name; after a global install it provides the `eddy` command.
